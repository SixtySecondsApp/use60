...
export function useApolloIntegration() {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useApolloIntegration] status error:', e);
      setIsConnected(false);
    } finally {
      setLoading(false);
    }
  ...
  const connectApiKey = useCallback(async (apiKey: string) => {
    ...
    if (error) throw new Error(error.message || 'Failed to save Apollo API key');
  ...
  const disconnect = useCallback(async () => {
    ...
    if (error) throw new Error(error.message || 'Failed to disconnect Apollo');
