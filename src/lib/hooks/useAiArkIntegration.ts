...
export function useAiArkIntegration() {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      ...
      console.error('[useAiArkIntegration] status error:', e);
      setIsConnected(false);
    } finally {
      setLoading(false);
    }
  ...
  const connectApiKey = useCallback(async (apiKey: string) => {
    ...
    if (error) throw new Error(error.message || 'Failed to save AI Ark API key');
    toast.success('AI Ark connected');
    await refreshStatus();
  ...
  const disconnect = useCallback(async () => {
    ...
    if (error) throw new Error(error.message || 'Failed to disconnect AI Ark');
    toast.success('AI Ark disconnected');
    await refreshStatus();
