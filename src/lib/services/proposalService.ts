...
// ==========================================
// Edit Distance Tracking (STY-004)
// ==========================================

/**
 * Proposal section shape used for edit-distance comparison.
 * Mirrors ProposalSection from ProposalPreview.tsx — kept as a local
 * interface so this service does not import from a component file.
 */
export interface ProposalSectionForDiff {
  id: string;
  title: string;
...
/**
 * Metrics returned from a section-level edit comparison.
 */
export interface ProposalEditMetrics {
  /** Arithmetic mean of all per-section normalized distances (0–1). */
  overall_distance: number;
  /** Per-section normalized edit distance, keyed by section id. */
 ...
 * Uses a two-row rolling buffer: O(m*n) time, O(min(m,n)) space.
 */
 ...
 * Normalizes by max(a.length, b.length).
 */
export function computeEditDistance(original: string, edited: string): number {
  ...
  return levenshtein(original, edited) / maxLen;
}

/**
 * Compare original and edited ProposalSection arrays.
 *
 * Sections are matched by `id`. Newly added or deleted sections are treated
 * as fully changed (distance 1.0). The `overall_distance` is the arithmetic
 * mean of all per-section distances so that long boilerplate sections do not
 * dominate the score.
 *
...
export function computeProposalEditMetrics(
  originalSections: ProposalSectionForDiff[],
  editedSections: ProposalSectionForDiff[],
): ProposalEditMetrics {
  const originalById = new Map(originalSections.map((s) => [s.id, s]));
  const editedById = new Map(editedSections.map((s) => [s.id, s]));
  ...
  for (const id of allIds) {
    ...
    if (!orig && edit) {
      ...
      // Compare title + content together with a null-byte separator so that
      // a title-only change still registers a non-zero distance.
...
/**
 * STY-004: Track edit distance on approved_edited proposals.
 *
 * Call this when a rep edits a proposal before approving it. The function:
 * 1. Computes per-section and overall edit metrics using Levenshtein distance.
 * 2. Persists the metrics into `proposals.style_config._edit_metrics` so they
 *    are available for analytics and future autopilot training.
 * 3. Records an `approved_edited` autopilot signal via the
 *    `autopilot-record-signal` edge function, populating `edit_distance` and
 *    `edit_fields` on the `autopilot_signals` row.
 *
 * This function is fire-and-forget safe — it catches all errors and shows a
 * toast only on non-critical failures. The caller (approval flow) should not
 * need to handle its errors.
 *
 * @param proposalId         - UUID of the proposal being approved.
 * @param originalSections   - Sections as generated by the AI pipeline (before edits).
 * @param editedSections     - Sections after the rep made changes in the UI.
 * @param signalContext      - Autopilot context required to record the signal.
 */
export async function trackProposalEditMetrics(
  proposalId: string,
  originalSections: ProposalSectionForDiff[],
  editedSections: ProposalSectionForDiff[],
  signalContext: {
  ...
  try {
    // 1. Compute edit metrics
    const metrics = computeProposalEditMetrics(originalSections, editedSections);

    // Derive human-readable section names for edit_fields using the edited
    // sections array (fall back to the original array for deleted sections).
    const allSectionsById = new Map<string, ProposalSectionForDiff>([
      ...originalSections.map((s): [string, ProposalSectionForDiff] => [s.id, s]),
      ...editedSections.map((s): [string, ProposalSectionForDiff] => [s.id, s]),
    ]);
    const editFields = metrics.edited_section_ids.map(
      (id) => allSectionsById.get(id)?.title ?? id,
    );

    // 2. Persist metrics into proposals.style_config._edit_metrics
    //    Read current style_config first so we don't clobber other keys.
    ...
    if (fetchError) {
      logger.error('[STY-004] Failed to fetch proposal style_config for edit metrics:', fetchError);
    } else {
      const currentStyleConfig =
        (existing?.style_config as Record<string, unknown> | null) ?? {};
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          style_config: {
            ...currentStyleConfig,
            _edit_metrics: {
              overall_distance: metrics.overall_distance,
              section_distances: metrics.section_distances,
              edited_section_ids: metrics.edited_section_ids,
              most_edited_section: metrics.most_edited_section,
      ...
      if (updateError) {
        logger.error('[STY-004] Failed to persist edit metrics to proposal:', updateError);
      }
    }

    // 3. Record approved_edited autopilot signal with edit_distance + edit_fields
    const { data: sessionData } = await supabase.auth.getSession();
    const token = sessionData.session?.access_token;
    if (!token) {
      logger.error('[STY-004] No auth token — cannot record autopilot signal');
      return;
    }

    const { error: signalError } = await supabase.functions.invoke('autopilot-record-signal', {
      method: 'POST',
      body: {
        action_type: signalContext.action_type,
        agent_name: signalContext.agent_name,
        signal: 'approved_edited',
        edit_distance: metrics.overall_distance,
        edit_fields: editFields,
        time_to_respond_ms: signalContext.time_to_respond_ms ?? null,
        confidence_at_proposal: signalContext.confidence_at_proposal ?? null,
        deal_id: signalContext.deal_id ?? null,
        contact_id: signalContext.contact_id ?? null,
        meeting_id: signalContext.meeting_id ?? null,
        autonomy_tier_at_time: signalContext.autonomy_tier_at_time,
      },
      headers: { Authorization: `Bearer ${token}` },
    });

    if (signalError) {
      logger.error('[STY-004] Failed to record autopilot signal for approved_edited:', signalError);
    }
  } catch (err) {
    logger.error('[STY-004] Unexpected error in trackProposalEditMetrics:', err);
  }
}
