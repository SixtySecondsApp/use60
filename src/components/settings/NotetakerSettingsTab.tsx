...
import type { RecordingRule, DomainMode, RecordingRuleInsert } from '@/lib/types/meetingBaaS'
...
export const NotetakerSettingsTab: React.FC = () => {
  ...
  React.useEffect(() => {
    if (calendarsData) {
      console.log('[NotetakerSettingsTab] Calendar data received:', calendarsData)
      console.log('[NotetakerSettingsTab] Calendars count:', calendarsData?.calendars?.length ?? 0)
    }
  }, [calendarsData])
  ...
  // Show Google Calendar dependency CTA if not connected
  ...
  return (
    <div className="space-y-6">
      {/* Connection Status Card — hidden once fully set up */}
      {!(googleConnected && hasMeetingBaaSCalendar) && (
...
export default NotetakerSettingsTab
