/**
 * AbilityMarketplace — Reusable Agent Abilities marketplace component
 *
 * Event-driven autonomous workflows organized by sales lifecycle stage.
 * Displays heartbeat status, lifecycle timeline, ability cards, and activity feed.
 */
...
function useAllAbilityPrerequisites() {
  // Get integration states — select primitives directly to avoid new-object re-render loops
  ...
  // Memoize the map to prevent re-creating on every render
  return useMemo(() => {
    ...
    for (const ability of ABILITY_REGISTRY) {
      ...
      for (const req of ability.requiredIntegrations) {
        ...
        switch (req.integrationId) {
          case 'slack': connected = slackConnected; break;
          case 'google-workspace': connected = googleConnected; break;
          case 'fathom': connected = fathomConnected; break;
          case 'instantly': connected = instantlyConnected; break;
        }
    ...
    return results;
  }, [slackConnected, googleConnected, fathomConnected, instantlyConnected]);
}

interface AbilityMarketplaceProps {
  showDebugTabs?: boolean; // default false — controls Run/Waves/History/Approvals tabs
}

export function AbilityMarketplace({ showDebugTabs = false }: AbilityMarketplaceProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-6 space-y-6">
      ...
      {/* Ability Run Panel + Detail Tabs (conditionally shown based on showDebugTabs) */}
      {selectedAbility && showDebugTabs && (
