...
function LogRow({ event, rates, fxRate }: LogRowProps) {
  const providerColor = PROVIDER_COLORS[event.provider] || 'text-slate-400';
  ...
  return (
    ...
    >
      ...
      {/* Username */}
      <span className="text-cyan-300 truncate" title={event.user_email || undefined}>
      ...
      <span className="truncate" title={dummy ? 'Estimated — tokens may be approximate' : undefined}>
        <span className={dummy ? 'text-slate-500' : 'text-indigo-300'}>{formatTokens(event.input_tokens)}</span>
        <span className="text-slate-600">/</span>
        <span className={dummy ? 'text-slate-500' : 'text-emerald-300'}>{formatTokens(event.output_tokens)}</span>
        {dummy && <span className="text-orange-400/60 ml-0.5" title="Hardcoded token count — not from API response">~</span>}
      </span>

      {/* Cost — computed in GBP from USD rates × FX */}
      <span
        className="text-yellow-300/80 text-right"
    ...
    return events.reduce(
      (acc, e) => {
        if (new Date(e.created_at).getTime() >= cutoff) {
          ...
          acc.cost += usd ?? e.estimated_cost ?? 0;
