...
import { DEFAULT_SANKEY_COLORS } from '@/components/goldeneye/ParticleFlowCanvas';
...
const COLORS = DEFAULT_SANKEY_COLORS;
...
function ChartTooltip({ active, payload }: ChartTooltipProps) {
  ...
  return (
    <div className="rounded-md border border-slate-700 bg-[#1f2937] px-3 py-2 shadow-xl">
      <div className="text-[10px] font-semibold text-slate-200">{d.name}</div>
  ...
  return (
    <div className="rounded-lg border border-slate-700/50 bg-slate-900/60 flex flex-col overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-slate-700/50">
        ...
        <span className="text-[10px] font-semibold text-slate-300 uppercase tracking-wider">Model Ratio</span>
      </div>

      <div className="flex-1 flex items-center px-2 py-2">
        {/* Donut chart */}
        <div className="w-[140px] h-[130px] shrink-0 relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-[11px] font-bold text-slate-200 font-mono">{formatCompact(totalTokens)}</span>
            <span className="text-[8px] text-slate-500 uppercase">tokens</span>
          </div>
        </div>
