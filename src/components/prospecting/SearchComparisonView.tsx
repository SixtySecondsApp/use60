    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <GitCompareArrows className="h-4 w-4 text-slate-500 dark:text-gray-400" />
          <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Comparison</span>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="grid grid-cols-2 gap-3">
        {[entryA, entryB].map((entry, idx) => (
          <div key={entry.id} className="rounded-lg border border-gray-200 dark:border-gray-700/50 p-3 space-y-2">
            <div className="flex items-center justify-between">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-1.5 text-xs text-gray-900 dark:text-gray-100">
                <Hash className="h-3 w-3 text-slate-500 dark:text-gray-400" />
      ...
      {/* Diff stats */}
      <div className="rounded-lg border border-gray-200 dark:border-gray-700/50 p-3 space-y-3">
        <div className="text-xs font-medium text-slate-500 dark:text-gray-400">Estimated Difference</div>

        <div className="grid grid-cols-3 gap-2 text-center">
          <div>
            <div className="text-lg font-semibold text-emerald-600 dark:text-emerald-400">{newInB}</div>
            <div className="text-xs text-slate-500 dark:text-gray-400">New in B</div>
          </div>
          <div>
            <div className="text-lg font-semibold text-blue-600 dark:text-blue-400">{estimatedOverlap}</div>
            <div className="text-xs text-slate-500 dark:text-gray-400">Overlap</div>
          </div>
          <div>
            <div className="text-lg font-semibold text-red-600 dark:text-red-400">{droppedFromA}</div>
            <div className="text-xs text-slate-500 dark:text-gray-400">Dropped from A</div>
          </div>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="h-2 rounded-full bg-slate-100 dark:bg-gray-700/50 overflow-hidden">
            <div
