...
import { ThemeToggleV8 } from './ThemeToggleV8';
...
export function NavbarV8({ onBookDemo, isDark, onToggleTheme }: NavbarV8Props) {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl shadow-[0_1px_0_0_rgba(0,0,0,0.06)] dark:shadow-[0_1px_0_0_rgba(255,255,255,0.06)]'
          : 'bg-transparent'
      }`}
    >
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <a
            ...
            className="hidden md:inline-flex text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors"
          >
            Log in
          </a>
          <ThemeToggleV8 isDark={isDark} onToggle={onToggleTheme} />
          <button
            onClick={onBookDemo}
          ...
          <button
            ...
            className="md:hidden p-2 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100 transition-colors"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="md:hidden overflow-hidden bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-gray-100 dark:border-white/10"
          >
            <div className="max-w-6xl mx-auto px-6 py-6 flex flex-col gap-4">
              {NAV_LINKS.map((link) => (
                <a
                  ...
                  className="text-base font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors py-2"
                >
                  {link.label}
                </a>
              ))}
              ...
              <button
                ...
                className="mt-2 w-full px-5 py-3 rounded-lg text-sm font-semibold
                  bg-blue-600 dark:bg-emerald-500 text-white hover:bg-blue-700 dark:hover:bg-emerald-600 transition-colors"
              >
                Book a demo
              </button>
