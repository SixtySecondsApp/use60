/**
 * InteractiveDemoV20 — Scroll-locked chapters + animation enhancements
 *
 * Changes from V17:
 * 1. SCROLL-LOCKED CHAPTERS (#6): Sticky scroll-driven section (600vh tall wrapper,
 *    sticky UI at top, chapters advance as user scrolls through). Vertical progress
 *    indicator alongside step cards.
 * 2. GREEN PULSE CASCADE (#7): Ripple on enrichment cell completion — bg-emerald-400/40,
 *    scale [1, 2.5], opacity [0.6, 0].
 * 3. RADAR PING (#8): Concentric rings on intent detection items (Analyze chapter).
 *    3 rings, staggered 200ms apart, border-2 border-emerald-400/30, scale 0.5->2.5.
 * 4. ENHANCED PROPOSAL (#1, #4): 60% viewport modal, page-flip reveal with
 *    perspective(800px) + rotateY, shimmer pass, "Proposal crafted" text.
 *
 * All 6 chapter demos preserved from V17, self-contained (no V17 imports).
 */
...
// ─── Shared: Enrichment Cell (with cascade pulse — Enhancement #2) ──────────
...
function EnrichCell({ state, value, className = '' }: { state: CellState; value: string; className?: string }) {
  ...
  // Enhancement #2: Green Pulse Cascade on 'done'
  return (
    <span className="relative inline-flex items-center">
      <motion.span
        ...
        transition={spring}
        className={`relative z-10 text-emerald-600 dark:text-emerald-400 font-medium text-[10px] truncate ${className}`}
      >
        {value}
      </motion.span>
      {/* Cascade pulse ripple — bg-emerald-400/40, scale [1, 2.5], opacity [0.6, 0] */}
      <motion.div
    ...
    <motion.div {...demoEnter} className="space-y-3">
      ...
      <div className="w-3/5 mx-auto">
        <div className="rounded-xl border border-[#E2E8F0] dark:border-gray-700/50 overflow-hidden">
          <div className="relative bg-gray-100 dark:bg-slate-900" style={{ aspectRatio: '16/9' }}>
            <div className="grid grid-cols-2 grid-rows-2 h-full gap-px bg-gray-200/50 dark:bg-white/[0.06]">
              {ATTENDEES.map((a, i) => (
                <div key={i} className="flex flex-col items-center justify-center bg-gray-100 dark:bg-slate-900 gap-1 relative">
            ...
            {/* Overlays */}
            <div className="absolute top-1.5 left-1.5 flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm border border-gray-200 dark:border-white/[0.06]">
              <div className={`w-1 h-1 rounded-full ${recording ? 'bg-red-500 animate-pulse' : complete ? 'bg-gray-500' : 'bg-gray-400'}`} />
              <span className={`text-[7px] font-semibold ${recording ? 'text-red-500 dark:text-red-400' : 'text-gray-500 dark:text-gray-400'}`}>{complete ? 'DONE' : recording ? 'REC' : 'READY'}</span>
            </div>
            <div className="absolute top-1.5 right-1.5 flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm text-[7px] text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-white/[0.06]">
...
//  action items & intent detection + RADAR PING (#8)
// ═══════════════════════════════════════════════════════════
...
/** Enhancement #8: Radar Ping component — concentric rings from the Zap icon */
function RadarPing() {
  return (
    <>
      {[0, 1, 2].map((ringIdx) => (
        <motion.div
          ...
          animate={{ scale: 2.5, opacity: 0 }}
          transition={{ delay: ringIdx * 0.2, duration: 1.2, ease: 'easeOut' }}
...
function AnalyzeMeetingsDemo({ isActive }: { isActive: boolean }) {
  ...
  // Track which intents have been "seen" so radar ping fires once per item
  const [seenIntents, setSeenIntents] = useState<Set<number>>(new Set());

  useEffect(() => {
    if (!isActive) { setPhase('loading'); setActionCount(0); setIntentCount(0); setSeenIntents(new Set()); return; }
    const t: ReturnType<typeof setTimeout>[] = [];
    ...
    INTENTS.forEach((_, i) => t.push(setTimeout(() => {
      setIntentCount(i + 1);
      setSeenIntents(prev => new Set([...prev, i]));
    }, 3500 + i * 350)));
    return () => t.forEach(clearTimeout);
  }, [isActive]);
  ...
  return (
    <motion.div {...demoEnter} className="space-y-3">
      ...
      {/* Intent Detection — Enhancement #8: Radar Ping on each intent item */}
      {phase === 'intents' && intentCount > 0 && (
        <div className="space-y-1.5">
          ...
          {INTENTS.slice(0, intentCount).map((intent, i) => {
            ...
            const justAppeared = seenIntents.has(i);
            return (
              <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                transition={{ delay: i * 0.05, ease: [0.22, 1, 0.36, 1] }}
                className={`flex items-center gap-2 p-2 rounded-lg ${colors.bg} border ${colors.border}`}>
                {/* Zap icon with radar ping rings */}
                <div className="relative w-3.5 h-3.5 flex items-center justify-center shrink-0">
                  {justAppeared && <RadarPing />}
                  <Zap className={`w-3.5 h-3.5 ${colors.text} relative z-10`} />
                </div>
                <div className="min-w-0">
...
// ═══════════════════════════════════════════════════════════
//  CHAPTER 5: Follow Up — Enhanced Proposal (#1, #4)
//  60% viewport modal, page-flip with perspective, shimmer
// ═══════════════════════════════════════════════════════════
...
function FollowUpDemo({ isActive }: { isActive: boolean }) {
  ...
  useEffect(() => {
    if (!isActive) { setStep(-1); setPageLines(0); setCoverReady(false); setDone(false); setShimmer(false); setPhase('intent'); setShowModal(false); return; }
    const t: ReturnType<typeof setTimeout>[] = [];
    ...
    // Done -> show PDF complete, then shimmer pass
    t.push(setTimeout(() => { setDone(true); setShimmer(true); }, 3400));
    t.push(setTimeout(() => setShimmer(false), 4200));
    t.push(setTimeout(() => setPhase('pdf'), 3400));
    // Email
  ...
  return (
    <motion.div {...demoEnter} className="relative">
      ...
      {/* Enhancement #4: Enhanced Modal overlay — 60% viewport, page-flip, shimmer */}
      <AnimatePresence>
        {showModal && (
          <motion.div
            ...
            className="absolute inset-x-1 top-2 bottom-1 z-10 rounded-xl border border-[#E2E8F0] dark:border-gray-700/50 bg-white dark:bg-gray-900 shadow-[0_8px_30px_rgba(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col"
          >
            {/* Modal header */}
            <div className="flex items-center justify-between px-3 py-2 border-b border-[#E2E8F0] dark:border-gray-700/50 bg-[#F8FAFC] dark:bg-gray-950/50">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] text-[#94A3B8]">Campium x 60</span>
              </div>
              <span className="text-[9px] font-mono text-[#94A3B8]">{progress}%</span>
            </div>

            {/* Horizontal progress bar for steps */}
            <div className="flex items-center gap-1 px-3 py-2 border-b border-[#E2E8F0] dark:border-gray-700/50 bg-white dark:bg-gray-900/80">
                ...
                return (
                  <div key={i} className="flex items-center gap-1">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 transition-all ${
                      ...
                      {done2 ? <Check className="w-2.5 h-2.5" /> : active2 ? <Loader2 className="w-2.5 h-2.5 animate-spin" /> : <s.icon className="w-2.5 h-2.5" />}
                    </div>
                    <span className={`text-[8px] font-medium hidden sm:inline ${active2 ? 'text-blue-600 dark:text-blue-400' : done2 ? 'text-gray-700 dark:text-gray-300' : 'text-[#94A3B8] dark:text-gray-600'}`}>{s.label}</span>
                    {i < PROPOSAL_STEPS.length - 1 && <div className="w-4 h-px bg-gray-200 dark:bg-gray-700" />}
                  </div>
                );
              })}
            </div>

            {/* Modal body — PDF takes ~60% of width, page-flip animation */}
              ...
              <AnimatePresence mode="wait">
                {(phase === 'modal' || phase === 'intent') && (
                  <motion.div key="generating" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, y: -6 }}
                    ...
                    {/* 2-page PDF document assembly — takes ~60% width, perspective container */}
                    <div className="flex gap-2 items-start relative" style={{ width: '60%', perspective: '800px' }}>
                      {/* Page 1: Cover — page-flip from rotateY(-90deg) to rotateY(0deg) */}
                      <motion.div
                        initial={{ rotateY: -90, opacity: 0 }}
                        animate={{ rotateY: coverReady ? 0 : -90, opacity: coverReady ? 1 : 0.4 }}
                        transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
                        style={{ transformOrigin: 'left center', transformStyle: 'preserve-3d' }}
                        className="flex-1 rounded border border-[#E2E8F0] dark:border-gray-700/30 shadow-sm overflow-hidden bg-white dark:bg-gray-800/30 relative"
                      >
                        ...
                        {/* Shimmer pass after both pages land */}
                        {shimmer && (
                          <motion.div
                            initial={{ x: '-100%' }}
                            animate={{ x: '200%' }}
                        ...
                        {/* Subtle shadow on flipping edge */}
                        <div className="absolute right-0 top-0 bottom-0 w-1 bg-gradient-to-l from-black/5 to-transparent pointer-events-none" />
                      </motion.div>

                      {/* Page 2: Content sections — page-flip with delay */}
                      <motion.div
                        initial={{ rotateY: -90, opacity: 0 }}
                        animate={{ rotateY: pageLines > 0 ? 0 : -90, opacity: pageLines > 0 ? 1 : 0.3 }}
                            ...
                            {pageLines > 0 ? (
                              <div className="space-y-1.5">
                                ...
                                {pageLines >= 5 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
                                  ...
                                  {[0, 1, 2].map((idx) => (
                                    <div key={idx} className={`flex gap-0.5 px-0.5 py-[1px] ${idx % 2 === 0 ? 'bg-white dark:bg-gray-800/30' : 'bg-gray-50 dark:bg-gray-800/50'}`}>
                ...
                {phase === 'pdf' && (
                  <motion.div key="pdf" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] font-semibold text-emerald-700 dark:text-emerald-400">Proposal crafted</p>
                ...
                {phase === 'email' && (
                  <motion.div key="email" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
                    ...
                    <div className="text-[9px] text-[#94A3B8] space-y-0.5">
                      ...
                      <p><span className="text-[#64748B] dark:text-gray-400">Subject:</span> Follow-up: 60 x Campium</p>
                    </div>
...
// ═══════════════════════════════════════════════════════════
//  Step Card Navigation (left sidebar) + vertical progress
// ═══════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════
//  Main Component — Scroll-locked chapters (#6)
// ═══════════════════════════════════════════════════════════

export function InteractiveDemoV20() {
  ...
  return (
    <div id="demo">
        <section ref={sectionRef} className="relative bg-gray-50/50 dark:bg-[#0a1020] overflow-hidden py-16 md:py-20">
          ...
          <div className="relative max-w-7xl mx-auto px-6 w-full py-6 md:py-8">
