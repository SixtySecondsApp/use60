...
/* ═══════════════════════════════════════════════════════════════
   use60 Hero V6 — "Dark Intelligence"
   Aesthetic: Linear meets Bloomberg Terminal meets Superhuman
   ═══════════════════════════════════════════════════════════════ */
...
const fonts = {
  ...
  heading: "'General Sans', 'Satoshi', -apple-system, sans-serif",
...
const EMAIL_LINES = [
  ...
  'next Tuesday as agreed. Looking forward to',
...
function AgentStep({ step, isVisible }: { step: typeof AGENT_STEPS[0]; isVisible: boolean; index: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-sm flex-shrink-0" style={{ color: tokens.textPrimary, fontFamily: fonts.body, fontWeight: 500 }}>
        {step.label}
      </span>
      <span className="flex-1 border-b border-dotted" style={{ borderColor: 'rgba(255,255,255,0.06)' }} />
      <span className="text-xs flex-shrink-0" style={{ color: tokens.textTertiary, fontFamily: fonts.mono, fontSize: 11 }}>
        {step.detail}
      </span>
    ...
    >
      ...
      <div className="px-4 py-2.5 flex items-center gap-3" style={{ borderBottom: `1px solid ${tokens.border}` }}>
        ...
        <div className="flex-1 min-w-0 flex items-center gap-2">
          <span style={{ color: tokens.textTertiary, fontFamily: fonts.mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em' }}>To:</span>
          <span style={{ color: tokens.textSecondary, fontFamily: fonts.mono, fontSize: 11 }}>sarah.chen@acme.com</span>
        </div>
        <span className="px-2 py-0.5 rounded text-xs font-medium" style={{ backgroundColor: tokens.accentGlow, color: tokens.accent, fontFamily: fonts.mono, fontSize: 10 }}>DRAFT</span>
      </div>

      {/* Subject */}
      <div className="px-4 py-2" style={{ borderBottom: `1px solid ${tokens.border}` }}>
        <span style={{ color: tokens.textTertiary, fontFamily: fonts.mono, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Subject: </span>
        <span style={{ color: tokens.textPrimary, fontFamily: fonts.body, fontSize: 12, fontWeight: 500 }}>
      ...
      <div className="px-4 py-3" style={{ minHeight: 120 }}>
        <pre style={{ color: tokens.textSecondary, fontFamily: fonts.body, fontSize: 12, lineHeight: 1.65, whiteSpace: 'pre-wrap', margin: 0 }}>
          {emailText}
  ...
  return (
    <div ref={ref} className="relative w-full max-w-xl mx-auto lg:mx-0">
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: `1px solid ${tokens.border}` }}>
          <div className="flex items-center gap-2.5">
            ...
            <span style={{ fontFamily: fonts.mono, fontSize: 12, fontWeight: 600, color: tokens.textPrimary, letterSpacing: '0.02em' }}>@60</span>
            <span style={{ fontFamily: fonts.mono, fontSize: 11, color: tokens.textTertiary }}>— processing call</span>
          </div>
          ...
          {visibleSteps.includes('flag') && (
            ...
            >
              <button className="px-3.5 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-all hover:scale-105" style={{
                backgroundColor: tokens.accent, color: '#fff', fontFamily: fonts.body,
              ...
              <button className="px-3.5 py-1.5 rounded-lg text-xs font-medium transition-colors hover:bg-white/5" style={{
                border: `1px solid ${tokens.border}`, color: tokens.textSecondary, fontFamily: fonts.body,
              }}>
                Edit First
              </button>
              <button className="px-3.5 py-1.5 rounded-lg text-xs font-medium transition-colors hover:bg-white/5 ml-auto" style={{
                color: tokens.textTertiary, fontFamily: fonts.body,
              }}>
                View Full Summary →
              </button>
            </motion.div>
      ...
      >
        ...
        <div>
          <div style={{ fontFamily: fonts.body, fontSize: 11, fontWeight: 600, color: tokens.textPrimary }}>HubSpot Updated</div>
          <div style={{ fontFamily: fonts.mono, fontSize: 9, color: tokens.textTertiary }}>Deal: Acme Corp • £42k</div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div>
          <div style={{ fontFamily: fonts.body, fontSize: 11, fontWeight: 600, color: tokens.textPrimary }}>Slack Posted</div>
          <div style={{ fontFamily: fonts.mono, fontSize: 9, color: tokens.textTertiary }}>#sales-team • 4 items</div>
        </div>
      </motion.div>
    </div>
  );
...
function CountUp({ target, suffix = '', isDecimal = false, isActive = false, delay = 0 }: {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const step = (ts: number) => {
        ...
        setValue((1 - Math.pow(1 - progress, 4)) * target);
  ...
  return (
    <span style={{
      ...
      color: tokens.textPrimary, letterSpacing: '-0.03em',
      ...
      {stats.map((stat, i) => (
        ...
        >
          <CountUp
            ...
            delay={i * 200}
          />
          <div style={{ fontFamily: fonts.body, fontSize: 11, color: tokens.textTertiary, marginTop: 4, lineHeight: 1.3 }}>
            {stat.label}
          </div>
        </motion.div>
      ))}
...
function TrustBar() {
  ...
  return (
    <motion.div
      ...
      className="mt-20 pt-8 pb-6"
      style={{ borderTop: `1px solid ${tokens.border}` }}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
          <span style={{ fontFamily: fonts.mono, fontSize: 11, color: tokens.textTertiary, letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500 }}>
            Integrates with
          </span>
          <div className="flex items-center gap-8">
            {TRUST_LOGOS.map((name, i) => (
              ...
              >
                <span style={{ fontFamily: fonts.heading, fontSize: 14, fontWeight: 600, color: tokens.textPrimary, letterSpacing: '-0.01em' }}>
                  {name}
                </span>
              </motion.div>
            ))}
            ...
            return (
              <motion.div
                ...
                style={{
                  ...
                  border: `1px solid ${tokens.border}`,
                  fontFamily: fonts.mono, fontSize: 11, fontWeight: 700, color: tokens.textSecondary,
                }}
        ...
        <motion.div animate={{ y: [0, -4, 0] }} transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}>
          <div className="w-48 rounded-xl overflow-hidden" style={{ backgroundColor: tokens.bgElevated, border: `1px solid ${tokens.border}` }}>
            <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: `1px solid ${tokens.border}` }}>
              ...
              <span style={{ fontFamily: fonts.mono, fontSize: 9, color: tokens.textTertiary }}>Follow-up ready</span>
            </div>
            <div className="p-3">
              ...
              >
                ...
                <span style={{ fontFamily: fonts.body, fontSize: 10, fontWeight: 600, color: accent }}>Approve</span>
              </motion.div>
            </div>
          </div>
        </motion.div>
...
function HowItWorks() {
  ...
  return (
    <section ref={ref} className="relative py-32" style={{ backgroundColor: tokens.bg }}>
      {/* Top separator line */}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          <span style={{ fontFamily: fonts.mono, fontSize: 11, color: tokens.accent, letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500 }}>
            How It Works
          </span>
          <h2 className="mt-4" style={{
            ...
            <span style={{ color: tokens.accent }}>three steps</span>
          </h2>
        </motion.div>
          ...
          {HOW_IT_WORKS_STEPS.map((step, i) => (
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden" style={{ background: `linear-gradient(135deg, ${step.accent}08, transparent)` }}>
                ...
                <div className="absolute top-4 right-4" style={{ fontFamily: fonts.mono, fontSize: 64, fontWeight: 700, color: `${step.accent}10`, lineHeight: 1 }}>
                  {step.num}
                </div>
              </div>
              ...
              <div className="p-6">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ backgroundColor: `${step.accent}15` }}>
                    <span style={{ fontFamily: fonts.mono, fontSize: 10, fontWeight: 700, color: step.accent }}>{step.num}</span>
                  </div>
                  <h3 style={{ fontFamily: fonts.heading, fontSize: 18, fontWeight: 700, color: tokens.textPrimary, letterSpacing: '-0.01em' }}>
                    {step.title}
                  </h3>
                </div>
                <p style={{ fontFamily: fonts.body, fontSize: 14, color: tokens.textSecondary, lineHeight: 1.65 }}>
                  {step.description}
                </p>
              </div>
            </motion.div>
              ...
              <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-xl">
                ...
                <motion.div variants={itemVariants}>
                  <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full" style={{
                    backgroundColor: tokens.accentGlow, border: '1px solid rgba(79,124,255,0.2)',
                  }}>
                    <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: tokens.accent }} />
                    <span style={{ fontFamily: fonts.mono, fontSize: 11, fontWeight: 500, color: tokens.accent, letterSpacing: '0.04em' }}>
                      Now in early access
                    </span>
                  </div>
                </motion.div>
                ...
                {/* Headline */}
                <motion.h1 variants={itemVariants} className="mt-8" style={{
                  fontFamily: fonts.display,
                  fontSize: 'clamp(42px, 5.5vw, 72px)',
                  fontWeight: 400, color: tokens.textPrimary,
                  ...
                  <br />
                  <span style={{
                    background: `linear-gradient(135deg, ${tokens.accent}, #7C3AED)`,
                ...
                {/* Sub */}
                <motion.p variants={itemVariants} className="mt-6" style={{
                  fontFamily: fonts.body, fontSize: 17, color: tokens.textSecondary, lineHeight: 1.65, maxWidth: 460,
                }}>
                  use60 turns your sales calls into closed deals — drafting follow-ups,
                  updating your CRM, and surfacing what needs attention. Automatically.
                </motion.p>

                {/* CTAs */}
                <motion.div variants={itemVariants} className="mt-10 flex flex-wrap items-center gap-3">
                  <button
                    className="group relative inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 hover:scale-[1.03] active:scale-[0.98]"
                    style={{
                      backgroundColor: tokens.accent, color: '#fff', fontFamily: fonts.body,
                      boxShadow: `0 0 24px ${tokens.accentGlow}, 0 1px 2px rgba(0,0,0,0.2)`,
                  ...
                  <button
                    ...
                    style={{ border: `1px solid ${tokens.border}`, color: tokens.textSecondary, fontFamily: fonts.body, letterSpacing: '-0.01em' }}
                  >
                    <Play size={14} />
                ...
                <motion.div variants={itemVariants} className="mt-8 flex flex-wrap items-center gap-5">
                  {[
                    ...
                    <div key={text} className="flex items-center gap-1.5">
                      ...
                      <span style={{ fontFamily: fonts.body, fontSize: 12, color: tokens.textTertiary, fontWeight: 500 }}>{text}</span>
                    </div>
                  ))}
                </motion.div>
