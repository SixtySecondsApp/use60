...
const STAGES = [
  ...
  { label: 'THE OUTREACH', key: 'outreach' },
] as const;
      ...
      <div className="flex items-center justify-center gap-[3px] h-8">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            ...
            style={{
              animation: isActive ? 'waveform-v11-workflow 0.8s ease-in-out infinite' : 'none',
...
function ActAfter({ isActive }: { isActive: boolean }) {
  ...
  useEffect(() => {
    ...
    const timers = [
      setTimeout(() => setShowProposal(true), 1800),
      setTimeout(() => setCheckedTasks([0]), 3000),
      setTimeout(() => setCheckedTasks([0, 1]), 3600),
      setTimeout(() => setCheckedTasks([0, 1, 2]), 4200),
      setTimeout(() => setShowSlack(true), 4800),
    ];
    return () => timers.forEach(clearTimeout);
  }, [isActive]);
    ...
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-mono font-medium text-blue-600 dark:text-emerald-400 tabular-nums">
              {healthWidth}%
            </span>
          </div>
        </div>
        <div className="w-full h-2 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
          <div
            className="h-full bg-blue-600 dark:bg-emerald-500 rounded-full transition-all duration-1000 ease-out"
            style={{ width: `${healthWidth}%` }}
          />
        </div>
        <div className="text-[10px] text-gray-400 dark:text-zinc-500 mt-1.5">Deal Health Score</div>
      </motion.div>
      ...
      <AnimatePresence>
        {showAction && (
          <motion.div
            ...
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
            className="bg-blue-50 dark:bg-emerald-500/5 border border-blue-200 dark:border-emerald-500/20 rounded-lg px-3 py-2.5"
          >
            <div className="text-[10px] font-semibold text-blue-600 dark:text-emerald-400 uppercase tracking-wider mb-1">
              Suggested Action
            </div>
            <p className="text-xs text-blue-800 dark:text-emerald-300 leading-relaxed">
      ...
      <AnimatePresence>
        {showLearning && (
          ...
          >
            <Brain className="w-3.5 h-3.5 text-blue-600 dark:text-emerald-400" />
...
function ActOutreach({ isActive }: { isActive: boolean }) {
  ...
  const prospects = [
    { name: 'Lisa Park', company: 'ScaleUp Inc', score: 94 },
    { name: 'Tom Bradley', company: 'GrowthOS', score: 91 },
    { name: 'Nina Vasquez', company: 'RevStack', score: 87 },
  ];
  ...
  useEffect(() => {
    ...
    const timers = [
      setTimeout(() => setVisibleProspects(1), 800),
      setTimeout(() => setVisibleProspects(2), 1400),
      setTimeout(() => setVisibleProspects(3), 2000),
      setTimeout(() => setShowSequence(true), 2800),
      setTimeout(() => setShowChannels(true), 3800),
      setTimeout(() => setShowStatus(true), 4800),
    ];
    return () => timers.forEach(clearTimeout);
  }, [isActive]);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <Zap className="w-4 h-4 text-blue-600 dark:text-emerald-400" />
        <span className="text-sm font-semibold text-gray-900 dark:text-white">
          60 finds new prospects that match your ICP and runs personalized outreach
        </span>
      </motion.div>

      {/* ICP Match card */}
      <div className="bg-blue-50 dark:bg-blue-500/5 border border-blue-200 dark:border-blue-500/20 rounded-lg px-3 py-2">
        <div className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-1">
          ICP Match
        </div>
        <div className="flex flex-wrap gap-1.5">
          {['SaaS', 'Series A-B', '50-200 employees', 'VP Sales'].map((tag) => (
            <span
              key={tag}
              className="text-[11px] font-medium text-blue-700 dark:text-blue-300 bg-blue-100 dark:bg-blue-500/10 px-2 py-0.5 rounded"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="space-y-2">
        {prospects.map((prospect, i) => (
          <motion.div
            key={prospect.name}
            initial={{ opacity: 0, x: -12 }}
            animate={i < visibleProspects ? { opacity: 1, x: 0 } : { opacity: 0, x: -12 }}
            transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
            className="flex items-center justify-between bg-gray-50 dark:bg-white/[0.03] border border-gray-100 dark:border-white/[0.06] rounded-lg px-3 py-2"
          >
            <div>
            ...
            <div className="flex items-center gap-1.5">
              <div className="text-xs font-mono font-medium text-blue-600 dark:text-emerald-400 tabular-nums">
                {prospect.score}%
              </div>
              <span className="text-[10px] text-gray-400 dark:text-zinc-500">match</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {showSequence && (
          ...
          >
            <div className="w-1.5 h-1.5 rounded-full bg-blue-600 dark:bg-emerald-500" />
            3-step sequence — Personalized per prospect
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showChannels && (
          ...
          >
            {[
              { label: 'Email', icon: Mail, color: 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' },
              { label: 'LinkedIn', icon: Linkedin, color: 'bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400' },
              { label: 'Video', icon: Video, color: 'bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400' },
            ].map((channel, i) => {
              ...
              return (
                <motion.span
                  ...
                  className={`text-[11px] font-medium px-2.5 py-1 rounded-full inline-flex items-center gap-1 ${channel.color}`}
                >
      ...
      <AnimatePresence>
        {showStatus && (
          ...
          >
            ...
            <span>12 new prospects engaged this week</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      {STAGES.map((stage) => (
        ...
        >
          ...
          <div className="p-5">
            ...
            {stage.key === 'outreach' && <ActOutreach isActive />}
          </div>
        </div>
      ))}
    </div>
...
export function WorkflowDemoV11() {
  ...
  return (
    ...
    >
      {/* Waveform keyframes */}
      <style>{`
        @keyframes waveform-v11-workflow {
          0%, 100% { height: 20%; }
          50% { height: 80%; }
        }
      `}</style>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <p className="mt-4 text-gray-500 dark:text-zinc-400 text-lg font-body max-w-2xl mx-auto">
            Watch how 60 works across every stage of the deal — from research to outreach.
          </p>
        </motion.div>
          ...
          >
            <div className="bg-white dark:bg-zinc-900/90 border border-gray-200 dark:border-white/[0.06] rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-white/[0.06]">
                ...
                {/* Progress segments — 5 clickable segments */}
                <div className="flex items-center gap-1.5">
                  {STAGES.map((stage, i) => (
                    <button
                      key={stage.key}
              ...
              <div className="p-5 min-h-[280px]">
                <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    {activeAct === 4 && <ActOutreach isActive={activeAct === 4} />}
                  </motion.div>
                </AnimatePresence>
              </div>
            </div>
