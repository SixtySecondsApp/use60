...
function SlackAlertAnimated({ isActive }: { isActive: boolean }) {
  ...
  useEffect(() => {
    ...
    const timers = [
      ...
      setTimeout(() => setShowConfirmation(true), 4000),
    ];
    return () => timers.forEach(clearTimeout);
  }, [isActive]);
        ...
        <AnimatePresence>
          {showConfirmation && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              transition={{ duration: 0.3 }}
              className="flex items-center gap-1.5 mt-3 text-[11px] text-gray-400 dark:text-zinc-500"
            >
...
/* ------------------------------------------------------------------ */
/*  Panel 4 — Proposal Generation (animated)                           */
/* ------------------------------------------------------------------ */

function ProposalAnimated({ isActive }: { isActive: boolean }) {
  ...
  useEffect(() => {
    ...
    const timers = [
      ...
      setTimeout(() => setVisibleSections(4), 2800),
      setTimeout(() => setVisibleSections(5), 3400),
    ];
    return () => timers.forEach(clearTimeout);
  }, [isActive]);
  ...
  return (
    <div>
      {/* Proposal header */}
      <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-100 dark:border-white/[0.06]">
        <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-emerald-500/10 flex items-center justify-center shrink-0">
          <FileText className="w-4 h-4 text-blue-600 dark:text-emerald-400" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-gray-900 dark:text-white">Proposal for MicroQuant</div>
          <div className="text-[10px] text-gray-400 dark:text-zinc-500">Generated 2 min after call ended</div>
        </div>
      </div>

      {/* Sections appearing progressively */}
      <div className="space-y-3">
        <AnimatePresence>
          {visibleSections >= 1 && (
            ...
            >
              <div className="text-[10px] font-semibold text-gray-400 dark:text-zinc-500 uppercase tracking-wider mb-1">
                Executive Summary
              </div>
              <p className="text-xs text-gray-600 dark:text-zinc-300 leading-relaxed">
                MicroQuant's 8-person sales team spends 14+ hours weekly on manual pipeline admin.
                60 eliminates this with AI-driven meeting prep, follow-ups, and deal intelligence.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {visibleSections >= 2 && (
            ...
            >
              ...
              <ul className="space-y-1">
                {[
                  'Full CRM integration with bi-directional HubSpot sync',
                  'AI meeting prep and auto follow-up for all reps',
                  'Pipeline intelligence with deal risk scoring',
                ].map((item) => (
                  <li key={item} className="flex items-start gap-2 text-xs text-gray-600 dark:text-zinc-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-blue-600 dark:bg-emerald-500 mt-1.5 shrink-0" />
                    {item}
                  </li>
                ))}
          ...
          {visibleSections >= 3 && (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-gray-900 dark:text-white">Investment: Custom Command Center</span>
                <span className="text-xs font-bold text-blue-600 dark:text-emerald-400">$2,400/mo</span>
              </div>
              <div className="flex items-center justify-between mt-1">
                <span className="text-[10px] text-gray-400 dark:text-zinc-500">Timeline</span>
                <span className="text-[10px] font-medium text-gray-600 dark:text-zinc-300">Live in 48 hours</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {visibleSections >= 4 && (
            ...
            >
              <span className="inline-flex items-center text-[10px] font-medium text-blue-600 dark:text-emerald-400 bg-blue-50 dark:bg-emerald-500/10 rounded-full px-2.5 py-1">
                Generated from call transcript
              </span>
              <span className="inline-flex items-center text-[10px] font-medium text-blue-600 dark:text-emerald-400 bg-blue-50 dark:bg-emerald-500/10 rounded-full px-2.5 py-1">
                Personalized to buyer
              </span>
            </motion.div>
          )}
        </AnimatePresence>
...
export function ShowcaseTabsV11() {
              ...
              {TABS.map((tab, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected && (
                      <motion.div
                        layoutId="tab-indicator-v11"
                        className="absolute bottom-0 left-0 right-0 h-[3px] bg-blue-600 dark:bg-emerald-500"
                    ...
                    {isSelected && (
                      <div className="absolute bottom-0 left-0 right-0 h-[3px]">
                        <div
                          className="h-full bg-blue-400/40 dark:bg-emerald-400/30"
            ...
            <div className="p-5 min-h-[300px]">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {activeTab === 3 && <ProposalAnimated isActive={activeTab === 3 && isInView} />}
                </motion.div>
              </AnimatePresence>
            </div>
          </div>
