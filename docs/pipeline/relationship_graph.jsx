...
const CONTACTS = [
  {
    ...
    timeline: [
      ...
      { type: "signal", label: "Opened proposal 3× in 24h", time: "2 days ago", sentiment: "hot" },
...
const SIGNAL_LABELS = {
  replied_fast: "Fast reply", opened_proposal: "Opened proposal", visited_pricing: "Pricing page",
  booked_demo: "Booked demo", shared_internally: "Shared internally", multi_thread: "Multi-thread",
  cc_decision_maker: "CC'd decision maker", linkedin_engaged: "LinkedIn active", budget_question: "Budget Q",
  page_view_docs: "Viewed docs", event_attended: "Event attended", cold_outbound_reply: "Outbound reply",
  video_viewed: "Watched video", website_lead: "Website lead", list_import: "List import", scraped_linkedin: "LinkedIn scrape",
};
...
const TIMELINE_ICONS = { email: "✉", meeting: "◎", signal: "◈", call: "☎", linkedin: "in" };

function getTier(w) { return WARMTH_TIERS.find(t => w >= t.min) || WARMTH_TIERS[3]; }
function fmt(n) { return n >= 1000 ? `£${(n / 1000).toFixed(0)}k` : `£${n}`; }

// ─── Agent Actions ─────────────────────────────────────────────
function getAgentActions(contact) {
  const company = COMPANIES.find(c => c.id === contact.company);
  const deal = contact.dealId ? DEALS.find(d => d.id === contact.dealId) : null;
  return [
    {
      id: "email", label: "Draft Follow-up", icon: "✉", color: "#22c55e", credits: 2,
      preview: {
        type: "email",
    ...
    {
      id: "prep", label: "Meeting Prep", icon: "◎", color: "#6366f1", credits: 4,
      preview: {
        type: "briefing",
        sections: [
          ...
          `Key signals: ${contact.signals.slice(0, 3).map(s => SIGNAL_LABELS[s]).join(", ")}`,
    ...
    {
      id: "reengage", label: contact.warmth > 0.5 ? "Keep Warm" : "Re-engage", icon: "↻", color: "#f97316", credits: 3,
      preview: {
        type: "sequence",
    ...
    {
      id: "task", label: "Create Task", icon: "✓", color: "#eab308", credits: 0,
      preview: {
        type: "task",
    ...
    {
      id: "enrich", label: "Enrich Profile", icon: "◈", color: "#8b5cf6", credits: 1,
      preview: {
        type: "enrich",
...
export default function RelationshipGraph() {
  ...
  const [actionFired, setActionFired] = useState(null);
  ...
  const nodes = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      items = items.filter(c => c.name.toLowerCase().includes(q) || COMPANIES.find(co => co.id === c.company)?.name.toLowerCase().includes(q));
    }
    if (filter !== "all") {
      const tier = WARMTH_TIERS.find(t => t.label.toLowerCase() === filter);
      if (tier) {
        const prev = WARMTH_TIERS[WARMTH_TIERS.indexOf(tier) - 1];
        items = items.filter(c => c.warmth >= tier.min && (!prev || c.warmth < prev.min));
      }
    }
    return items.map((contact, i) => {
      ...
      return { ...contact, tier, company, deal, angle, radius, x: 0, y: 0 };
    });
  }, [filter, searchQuery]);
  ...
  useEffect(() => {
    ...
    const particles = Array.from({ length: 90 }, () => ({
      ...
      size: Math.random() * 1.2 + 0.2, speed: Math.random() * 0.25 + 0.05,
      opacity: Math.random() * 0.25 + 0.03, hue: Math.random() > 0.6 ? 270 : Math.random() > 0.3 ? 230 : 200,
    }));

    const comets = nodes.filter(n => n.warmthDelta > 0.03).map(n => ({
      id: n.id, trail: [], maxTrail: 14,
    }));
    ...
    function draw() {
      ...
      particles.forEach(p => {
        p.y -= p.speed;
        p.x += Math.sin(t + p.y * 0.008) * 0.12;
        if (p.y < 0) { p.y = dims.h; p.x = Math.random() * dims.w; }
        const flicker = 0.3 + Math.sin(t * 2.5 + p.x * 0.4) * 0.7;
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.size * flicker, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(${p.hue}, 60%, 72%, ${p.opacity * flicker})`;
        ctx.fill();
      });
      ...
      // Center core with multi-ring pulse
      ...
      // Deal arcs
      const dealGroups = {};
      positions.forEach(n => { if (n.dealId) { (dealGroups[n.dealId] = dealGroups[n.dealId] || []).push(n); } });
      ...
      positions.forEach(node => {
        ...
        else if (isHov || isMatch) { ctx.strokeStyle = "rgba(255,255,255,0.5)"; ctx.lineWidth = 1.5; ctx.stroke(); }

        // Delta arrow
        if (Math.abs(node.warmthDelta) > 0.02) {
          const ax = node.x + size * 0.72, ay = node.y - size * 0.72;
        ...
        // Company badge
        if (node.company) {
          const bx = node.x - size * 0.68, by = node.y + size * 0.68;
          ctx.beginPath();
          ctx.arc(bx, by, 6.5, 0, Math.PI * 2);
          ctx.fillStyle = node.company.color;
          ctx.fill();
          ...
          ctx.textAlign = "center";
          ctx.textBaseline = "middle";
          ctx.fillText(node.company.logo, bx, by);
        }
        ...
        if (node.warmth > 0.42 || isHov || isSel || isMatch) {
          ...
          if (isHov || isSel) {
            ...
            ctx.fillText(`${node.role} · ${node.company?.name}`, node.x, node.y + size + 24);
          }
        }
      });
  ...
  }, [dims, nodes, selected, hovered, showOrbits, zoom, pan, searchQuery]);
  ...
  const hitTest = useCallback((mx, my) => {
    ...
    for (let i = nodesRef.current.length - 1; i >= 0; i--) {
      ...
      const s = 15 + n.warmth * 11 + 5;
      if ((ex - n.x) ** 2 + (ey - n.y) ** 2 < s * s * 1.8) return n;
    }
    return null;
  }, [dims, zoom, pan]);
  ...
  const handleClick = useCallback(e => {
    ...
    if (!rect) return;
    const node = hitTest(e.clientX - rect.left, e.clientY - rect.top);
    if (node) { setSelected(node); setPanelTab("overview"); setExpandedAction(null); setActionFired(null); }
    else if (!dragging.current) setSelected(null);
  }, [hitTest]);
  ...
  const handleMouseUp = useCallback(() => {
    ...
    canvasRef.current && (canvasRef.current.style.cursor = hovered ? "pointer" : "grab");
  }, [hovered]);

  const fireAction = (action) => { setActionFired(action.id); setTimeout(() => setActionFired(null), 3000); };

  const sc = selected;
  const sCompany = sc ? COMPANIES.find(c => c.id === sc.company) : null;
  const sDeal = sc?.dealId ? DEALS.find(d => d.id === sc.dealId) : null;
  const sRelated = sc ? CONTACTS.filter(c => c.company === sc.company && c.id !== sc.id) : [];
  const sActions = sc ? getAgentActions(sc) : [];

  const stats = useMemo(() => ({
    hot: CONTACTS.filter(c => c.warmth >= 0.7).length,
    warm: CONTACTS.filter(c => c.warmth >= 0.4 && c.warmth < 0.7).length,
    cool: CONTACTS.filter(c => c.warmth >= 0.15 && c.warmth < 0.4).length,
    cold: CONTACTS.filter(c => c.warmth < 0.15).length,
    pipeline: DEALS.reduce((s, d) => s + d.value, 0),
    trending: CONTACTS.filter(c => c.warmthDelta > 0.03).length,
  }), []);

  const glass = { background: "rgba(17,17,24,0.82)", backdropFilter: "blur(16px)", WebkitBackdropFilter: "blur(16px)" };
    ...
    <div style={{ width: "100%", height: "100vh", background: "#030712", fontFamily: "'DM Sans','Inter',-apple-system,sans-serif", color: "#e2e8f0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ ...glass, padding: "10px 18px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid rgba(99,102,241,0.10)", flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 5, alignItems: "center" }}>
          {[{ l: "Hot", c: stats.hot, clr: "#f97316" }, { l: "Warm", c: stats.warm, clr: "#eab308" }, { l: "Cool", c: stats.cool, clr: "#6366f1" }, { l: "Cold", c: stats.cold, clr: "#475569" }].map(s => (
            <button key={s.l} onClick={() => setFilter(f => f === s.l.toLowerCase() ? "all" : s.l.toLowerCase())} style={{
              display: "flex", alignItems: "center", gap: 4, padding: "3px 9px", borderRadius: 5,
              background: filter === s.l.toLowerCase() ? `${s.clr}22` : `${s.clr}0a`, border: filter === s.l.toLowerCase() ? `1px solid ${s.clr}50` : `1px solid ${s.clr}18`,
              fontSize: 10, fontWeight: 600, color: s.clr, cursor: "pointer", transition: "all 0.2s",
            }}><span style={{ width: 5, height: 5, borderRadius: "50%", background: s.clr }} />{s.c}</button>
          ))}
          ...
          {stats.trending > 0 && <div style={{ padding: "3px 10px", borderRadius: 5, background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.20)", fontSize: 10, fontWeight: 600, color: "#86efac" }}>↑ {stats.trending} trending</div>}
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        <div ref={containerRef} style={{ flex: 1, position: "relative" }}>
          ...
          {hovered && !selected && (
            <div style={{ position: "fixed", left: mouse.x + 16, top: mouse.y - 12, ...glass, border: `1px solid ${hovered.tier.color}25`, borderRadius: 10, padding: "10px 14px", pointerEvents: "none", zIndex: 100, minWidth: 210 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div><div style={{ fontSize: 12, fontWeight: 600 }}>{hovered.name}</div><div style={{ fontSize: 10, color: "#64748b" }}>{hovered.role} · {hovered.company?.name}</div></div>
              </div>
          ...
          {!selected && <div style={{ position: "absolute", bottom: 14, left: "50%", transform: "translateX(-50%)", fontSize: 10, color: "#475569", ...glass, padding: "5px 14px", borderRadius: 20, border: "1px solid rgba(42,42,58,0.20)" }}>Click to inspect · Scroll to zoom · Drag to pan · {nodes.length} contacts</div>}
        </div>
          ...
          <div style={{ width: 360, flexShrink: 0, borderLeft: "1px solid rgba(99,102,241,0.10)", ...glass, overflowY: "auto", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(42,42,58,0.30)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div><div style={{ fontSize: 15, fontWeight: 700 }}>{sc.name}</div><div style={{ fontSize: 11, color: "#64748b" }}>{sc.role} · {sCompany?.name}</div></div>
                </div>
                <button onClick={() => setSelected(null)} style={{ background: "rgba(42,42,58,0.4)", border: "none", color: "#64748b", width: 24, height: 24, borderRadius: 6, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
              </div>
            ...
            <div style={{ display: "flex", borderBottom: "1px solid rgba(42,42,58,0.30)" }}>
              {["overview", "timeline", "agents"].map(tab => (
                <button key={tab} onClick={() => { setPanelTab(tab); setExpandedAction(null); }} style={{
                  flex: 1, padding: "8px 0", fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em",
            ...
            {panelTab === "overview" && (
              <div style={{ flex: 1, overflowY: "auto" }}>
                <div style={{ padding: "10px 16px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 5 }}>
                  {[["Meetings", sc.meetings], ["Emails", sc.emails], ["Calls", sc.calls], ["LinkedIn", sc.linkedinMsgs]].map(([l, v]) => (
                    <div key={l} style={{ padding: "6px 8px", borderRadius: 6, background: "rgba(42,42,58,0.12)", border: "1px solid rgba(42,42,58,0.20)", textAlign: "center" }}>
                      <div style={{ fontSize: 14, fontWeight: 700 }}>{v}</div>
                      <div style={{ fontSize: 8, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: 1 }}>{l}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ padding: "0 16px 12px" }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                    {sc.signals.map(s => <span key={s} style={{ fontSize: 10, padding: "3px 7px", borderRadius: 5, background: "rgba(99,102,241,0.07)", border: "1px solid rgba(99,102,241,0.10)", color: "#a5b4fc", fontWeight: 500 }}>{SIGNAL_LABELS[s]}</span>)}
                  </div>
                </div>
                {sRelated.length > 0 && (
                  <div style={{ padding: "0 16px 12px" }}>
                    ...
                    {sRelated.map(rc => {
                      const rt = getTier(rc.warmth);
                      return (
                        <div key={rc.id} onClick={() => { setSelected(rc); setPanelTab("overview"); }}
                          ...
                          onMouseOver={e => e.currentTarget.style.background = "rgba(42,42,58,0.20)"}
                          onMouseOut={e => e.currentTarget.style.background = "transparent"}>
                          <div style={{ width: 22, height: 22, borderRadius: 5, background: `${rt.color}55`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 600, color: "#e2e8f0" }}>{rc.avatar}</div>
                          <div><div style={{ fontSize: 11, fontWeight: 500 }}>{rc.name}</div><div style={{ fontSize: 9, color: "#64748b" }}>{rc.role}</div></div>
                          <div style={{ marginLeft: "auto", fontSize: 10, fontWeight: 700, color: rt.color }}>{Math.round(rc.warmth * 100)}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div style={{ padding: "0 16px 16px" }}>
                  <div style={{ padding: "10px 12px", borderRadius: 8, background: "rgba(99,102,241,0.05)", border: "1px solid rgba(99,102,241,0.12)", display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div><div style={{ fontSize: 9, color: "#64748b", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>Next Step</div><div style={{ fontSize: 12, fontWeight: 600, marginTop: 1 }}>{sc.nextAction}</div></div>
                  </div>
                </div>
              </div>
            )}
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px" }}>
                ...
                <div style={{ position: "relative", paddingLeft: 20 }}>
                  ...
                  {sc.timeline.map((ev, i) => {
                    const sent = SENTIMENT_CONFIG[ev.sentiment] || SENTIMENT_CONFIG.neutral;
                    return (
                      <div key={i} style={{ marginBottom: 14, position: "relative" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                          <span style={{ fontSize: 10, color: "#64748b", fontWeight: 500 }}>{TIMELINE_ICONS[ev.type] || "·"} {ev.type[0].toUpperCase() + ev.type.slice(1)}</span>
                          <span style={{ fontSize: 9, color: "#475569" }}>{ev.time}</span>
                        </div>
                        <div style={{ fontSize: 12, fontWeight: 500, marginTop: 3 }}>{ev.label}</div>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 3, marginTop: 4, fontSize: 9, color: sent.color, background: `${sent.color}10`, padding: "2px 6px", borderRadius: 4 }}>{sent.icon} {ev.sentiment}</div>
                      </div>
                    );
                  })}
                        ...
                        {isExp && !isFired && (
                          <div style={{ padding: "0 12px 12px" }}>
                            ...
                            <div style={{ background: "rgba(3,7,18,0.45)", borderRadius: 7, padding: "10px 12px", border: "1px solid rgba(42,42,58,0.25)" }}>
                              {action.preview.type === "email" && (<><div style={{ fontSize: 10, color: "#64748b", marginBottom: 4 }}>Subject: <span style={{ color: "#94a3b8" }}>{action.preview.subject}</span></div><div style={{ fontSize: 11, color: "#cbd5e1", lineHeight: 1.5, whiteSpace: "pre-wrap", maxHeight: 110, overflow: "auto" }}>{action.preview.body}</div></>)}
                              {action.preview.type === "briefing" && action.preview.sections.map((s, i) => <div key={i} style={{ fontSize: 11, color: i === 0 ? "#e2e8f0" : "#94a3b8", marginBottom: 3, fontWeight: i === 0 ? 600 : 400 }}>{s}</div>)}
                              {action.preview.type === "sequence" && (<><div style={{ fontSize: 11, fontWeight: 600, marginBottom: 5, color: action.color }}>{action.preview.name}</div>{action.preview.steps.map((step, i) => <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 3 }}><div style={{ width: 15, height: 15, borderRadius: 4, background: "rgba(42,42,58,0.35)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, color: "#64748b", fontWeight: 600, flexShrink: 0 }}>{i + 1}</div><span style={{ fontSize: 10, color: "#94a3b8" }}>{step}</span></div>)}</>)}
                              {action.preview.type === "task" && (<><div style={{ fontSize: 12, fontWeight: 600 }}>{action.preview.title}</div><div style={{ display: "flex", gap: 5, marginTop: 5 }}><Tag bg="rgba(234,179,8,0.08)" c="#fde047">Due: {action.preview.due}</Tag><Tag bg="rgba(99,102,241,0.08)" c="#a5b4fc">{action.preview.priority}</Tag></div></>)}
                              {action.preview.type === "enrich" && (<><div style={{ fontSize: 10, color: "#64748b", marginBottom: 3 }}>Sources: {action.preview.sources.join(", ")}</div><div style={{ fontSize: 10, color: "#94a3b8" }}>Fields: {action.preview.fields.join(", ")}</div></>)}
                            </div>
...
// ─── Atoms ─────────────────────────────────────────────────────
function TipStat({ l, v, c }) {
  return <div><div style={{ fontSize: 8, color: "#475569", textTransform: "uppercase", letterSpacing: "0.04em" }}>{l}</div><div style={{ fontSize: 11, fontWeight: 600, color: c || "#e2e8f0", marginTop: 1 }}>{v}</div></div>;
}
...
function ZBtn({ onClick, children, wide }) {
