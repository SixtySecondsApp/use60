...
- Auto-detects dev server port from project config (not hardcoded)

---

## CRITICAL: AUTO-CHAIN RULE

**When invoked from 60/consult, after generating the PRD you MUST immediately invoke `/60-plan` using the Skill tool, which will then invoke `/60-run`. DO NOT tell the user to run these — the pipeline runs automatically.**

---

## Step 0: Model Profile Selection (FIRST STEP)

**If invoked as part of a chain** (from 60/consult), inherit the already-selected profile and skip this prompt.

[Uses AskUserQuestion tool:]

Question: "Select your model profile for PRD generation:"
Options:
  - Economy — Quick PRD with basic story breakdown.
  - Balanced (Recommended) — Thorough PRD with well-sized stories.
  - Thorough — Deep PRD with comprehensive edge cases.
...
- [ ] Another criterion
- [ ] Typecheck passes
- [ ] **[UI stories only]** Verify in browser on dev server (auto-detect port)
...
- `Verify in browser on dev server (auto-detect port)` (UI stories)

---

## use60 TECH STACK REFERENCE

- **Frontend**: React 18 + Vite (dev server (auto-detect port))
