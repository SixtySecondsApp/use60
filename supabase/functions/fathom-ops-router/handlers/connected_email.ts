/**
 * Handler: connected_email
 * Extracted from fathom-connected-email edge function.
 *
 * Sends notification when Fathom integration is successfully connected.
...
interface FathomConnectedEmailRequest {
  user_id: string;
...
function verifySecret(req: Request): boolean {
  ...
  if (!secret) {
    console.warn('[fathom-connected-email] No EDGE_FUNCTION_SECRET configured');
...
export async function handleConnectedEmail(req: Request): Promise<Response> {
  // Verify authentication
  if (!verifySecret(req)) {
    console.error('[fathom-connected-email] Authentication failed');
    return new Response(
  ...
  try {
    const {
      ...
      organization_name,
      action_url,
    }: FathomConnectedEmailRequest = await req.json();
    ...
    if (profileError || !profile) {
      console.error('[fathom-connected-email] Profile not found:', profileError);
      return new Response(
    ...
    const defaultActionUrl = action_url || `https://app.use60.com/organization/${organization_id}/analytics`;
    ...
    console.log('[fathom-connected-email] Delegating to encharge-send-email dispatcher');
    ...
    if (!dispatcherResponse.ok) {
      ...
      console.error('[fathom-connected-email] Dispatcher error:', errorText);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    console.log('[fathom-connected-email] Email sent successfully - Message ID:', dispatcherResult.message_id);
    ...
    console.error('[fathom-connected-email] Error:', error);
    return new Response(
      JSON.stringify({
        success: false,
