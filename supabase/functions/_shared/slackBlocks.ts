...
/**
 * Proposal Briefing — Full briefing pack posted to deal room (PDR-004)
 */
export interface ProposalBriefingData {
...
export const buildProposalBriefingMessage = (data: ProposalBriefingData): SlackMessage => {
  const blocks: SlackBlock[] = [];

  // Header
  blocks.push({
    type: 'header',
    text: { type: 'plain_text', text: safeHeaderText('New Proposal Ready'), emoji: true },
  });

  // Title + company + contact
  const titleParts = [`*${safeMrkdwn(data.proposalTitle)}*`];
  if (data.companyName) titleParts.push(data.companyName);
  if (data.contactName) titleParts.push(data.contactName);
  const valueStr = data.dealValue > 0
    ? ` | ${formatCurrency(data.dealValue, data.currencyCode, data.currencyLocale)}`
    : '';
  blocks.push(section(safeMrkdwn(`${titleParts.join(' · ')}${valueStr}`)));

  blocks.push(divider());

  // Meeting highlights
  if (data.meetingHighlights.length > 0) {
    const highlights = data.meetingHighlights
      .slice(0, 3)
      .map((h) => `• ${truncate(h, 200)}`)
      .join('\n');
    blocks.push(section(safeMrkdwn(`*Meeting Highlights*\n${highlights}`)));
  }

  // Next steps
  if (data.nextSteps) {
    blocks.push(section(safeMrkdwn(`*Next Steps*\n${data.nextSteps}`)));
  }
  ...
  if (data.actionItems.length > 0) {
    const items = data.actionItems
      .slice(0, 5)
      .map((a) => `☐ ${truncate(a, 150)}`)
      .join('\n');
    blocks.push(section(safeMrkdwn(`*Action Items*\n${items}`)));
  }

  blocks.push(divider());

  // Buttons
  const buttonDefs: Array<{ text: string; actionId: string; value: string; url?: string; style?: string }> = [];
  if (data.pdfUrl) {
    buttonDefs.push({
      text: 'Download PDF',
      actionId: 'download_proposal_pdf',
      value: safeButtonValue(data.proposalId),
      url: data.pdfUrl,
      style: 'primary',
    });
  }
  if (data.dealId) {
    buttonDefs.push({
      text: 'View Deal',
      actionId: 'view_deal',
      value: safeButtonValue(data.dealId),
      url: `${data.appUrl}/deals/${data.dealId}`,
    });
  }
  if (buttonDefs.length > 0) {
    blocks.push(actions(buttonDefs));
  }

  // Context footer
  const triggerLabel = data.triggerType === 'auto_post_meeting' ? 'Auto (post-meeting)'
    : data.triggerType === 'manual_button' ? 'Manual'
    : data.triggerType === 'copilot' ? 'Copilot'
    : data.triggerType === 'slack' ? 'Slack'
    : data.triggerType;
  blocks.push(context([safeContextMrkdwn(`Trigger: ${triggerLabel} · Generated by 60`)]));

  return {
    blocks,
    text: `New proposal ready: ${data.proposalTitle}`,
  };
};
