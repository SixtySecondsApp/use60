/**
 * Handler: generate-follow-up
 * Generates follow-up emails with SSE streaming.
 * Full logic extracted from generate-follow-up/index.ts.
 */

// Re-export the original module's handler directly.
// The follow-up function uses SSE streaming and GET/POST methods,
// which makes it incompatible with the router's JSON-body-based dispatch.
// Instead, we re-export the serve callback as a handler function.
...
export async function handleFollowUp(req: Request): Promise<Response> {
  ...
  // When called via generate-router, GET requests arrive as POST with { list_meetings: true }
  if (req.method === 'GET') {
  ...
  // Check if this is a list_meetings request routed via the generate-router (POST with action)
  const clonedReq = req.clone();
  try {
    const body = await clonedReq.json();
    if (body.list_meetings) {
      return handleRecentMeetings(req, corsHeaders);
    }
  } catch {
    // If body parsing fails, proceed to generate
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        sendEvent('step', { id: 'load_analysis', status: 'running', label: 'Loading transcript analysis' });
        ...
        const analysis = {
          ...
          actionItems: (actionItems ?? []).map((ai: { title: string | null; assignee_name: string | null }) => ({
            task: ai.title ?? '',
        ...
        sendEvent('step', {
          id: 'load_analysis', status: 'complete',
        ...
        sendEvent('step', { id: 'load_attendees', status: 'running', label: 'Loading attendees' });
        ...
        let primaryAttendee: { name: string | null; email: string | null } | undefined = extAttendees?.[0];
        ...
        sendEvent('step', {
          id: 'load_attendees', status: 'complete',
          label: `Primary: ${primaryAttendee.name ?? primaryAttendee.email}`,
        });

        sendEvent('step', { id: 'history_check', status: 'running', label: 'Checking meeting history' });

        const meetingHistory = await detectMeetingHistory(
          supabase, meeting_id, meeting.company_id as string | null, meeting.org_id as string,
        );

        sendEvent('step', {
          id: 'history_check', status: 'complete',
        ...
        let followUpContext: {
          hasHistory: boolean; meetingNumber: number;
          sections: Record<string, { chunks: unknown[] }>; queryCredits: number;
        } | null = null;

        if (cached_rag_context) {
          followUpContext = cached_rag_context;
          sendEvent('step', {
            id: 'rag_queries', status: 'complete',
          ...
          sendEvent('step', {
            id: 'rag_queries', status: 'running',
            label: `Querying history across ${meetingHistory.priorMeetingCount} meetings`,
          });

          const ragClient = createRAGClient({ orgId: meeting.org_id as string });

          followUpContext = await getFollowUpContext(
            null, [], meeting_id, meetingHistory.priorMeetingCount + 1,
            ragClient, meeting.company_id as string | null,
          );
          ...
          sendEvent('step', {
            id: 'rag_queries', status: 'skipped',
            label: 'First meeting — composing from transcript only',
          });
        }

        sendEvent('step', { id: 'writing_style', status: 'running', label: 'Loading rep writing style' });
        ...
        sendEvent('step', {
          ...
          label: writingStyle ? `Style loaded: ${(writingStyle as any).name}` : 'No writing style available',
        });
        ...
        if (resolvedDealId && meeting.org_id) {
          try {
            sendEvent('step', { id: 'deal_memory', status: 'running', label: 'Loading deal memory context' });

            const memoryRagClient = new RAGClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, meeting.org_id as string);
            const memoryReader = createDealMemoryReader(supabase, memoryRagClient);
            dealMemoryContext = await memoryReader.getDealContext(resolvedDealId, meeting.org_id as string);

            const commitmentCount = dealMemoryContext.openCommitments.length;
            const eventCount = dealMemoryContext.recentEvents.length;

            sendEvent('step', {
              id: 'deal_memory', status: 'complete',
              label: `${commitmentCount} commitments, ${eventCount} recent events`,
            });
          } catch (memErr) {
            console.error('[generate-follow-up] deal memory load failed (non-fatal):', memErr instanceof Error ? memErr.message : String(memErr));
            sendEvent('step', { id: 'deal_memory', status: 'skipped', label: 'Deal memory unavailable' });
          }
        }

        sendEvent('step', { id: 'compose', status: 'running', label: 'Composing follow-up email' });
        ...
        const composeInput = {
          ...
          deal: dealMemoryContext ? {
            commitments: dealMemoryContext.openCommitments.map(c => ({
              owner: c.owner, action: c.action, deadline: c.deadline, status: c.status,
            })),
            recentEvents: dealMemoryContext.recentEvents.slice(0, 10).map(e => ({
              type: e.event_type, summary: e.summary, date: e.source_timestamp,
            })),
        ...
        sendEvent('step', {
          id: 'compose', status: 'complete',
          label: `${email.wordCount} words`, detail: `${email.wordCount} words`,
        });

        let emailWithoutHistory: { to: string; subject: string; body: string; wordCount: number } | null = null;

        if (include_comparison && followUpContext?.hasHistory) {
          sendEvent('step', { id: 'compose_comparison', status: 'running', label: 'Composing comparison (without history)' });
          emailWithoutHistory = await composeFirstMeetingFollowUp(composeInput);
          sendEvent('step', {
            id: 'compose_comparison', status: 'complete',
        ...
        if (delivery === 'slack') {
          ...
          if (slackOrg?.bot_access_token && slackMapping?.slack_user_id) {
            const truncatedBody = email.body.length > 2500 ? email.body.slice(0, 2497) + '...' : email.body;

            const blocks = [
              { type: 'header', text: { type: 'plain_text', text: 'Follow-Up Email Ready for Review', emoji: true } },
              { type: 'divider' },
              { type: 'section', fields: [
                { type: 'mrkdwn', text: `*To:*\n${email.to}` },
              ...
              { type: 'actions', elements: [
                { type: 'button', text: { type: 'plain_text', text: 'Approve & Send', emoji: true }, style: 'primary', action_id: 'followup_approve', value: JSON.stringify({ meeting_id, to: email.to, subject: email.subject }) },
                { type: 'button', text: { type: 'plain_text', text: 'Edit in App', emoji: true }, action_id: 'followup_edit', value: meeting_id },
                { type: 'button', text: { type: 'plain_text', text: 'Dismiss', emoji: true }, action_id: 'followup_dismiss', value: meeting_id },
              ]},
            ];
            ...
            sendEvent('step', {
              id: 'slack_delivery', status: 'failed',
              label: !slackOrg?.bot_access_token ? 'Slack not connected for this org' : 'No Slack user mapping found',
            });
          }
        }

        const sectionsReturned = followUpContext ? Object.keys(followUpContext.sections).length : 0;
        ...
        const result = {
          ...
          email: {
            ...
            contextUsed: {
              ...
              commitmentsFound: (followUpContext?.sections?.['prior_commitments']?.chunks?.length as number) ?? 0,
              concernsFound: (followUpContext?.sections?.['prospect_concerns']?.chunks?.length as number) ?? 0,
        ...
        console.error('[generate-follow-up] stream error:', err instanceof Error ? err.message : String(err));
        try {
          const encoder2 = new TextEncoder();
          controller.enqueue(
            encoder2.encode(`event: error\ndata: ${JSON.stringify({ message: err instanceof Error ? err.message : 'Generation failed' })}\n\n`),
          );
        } catch (_) {
