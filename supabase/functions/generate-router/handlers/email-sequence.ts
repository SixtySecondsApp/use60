/**
 * Handler: generate-email-sequence
 * Two-tier email generation (Claude Sonnet + Gemini Flash).
 * Re-exports the full logic from the original edge function.
 */
  ...
  for (let i = 1; i <= numSteps; i++) {
    ...
    if (!subjectId) {
      ...
      columnsToCreate.push({
        table_id: tableId, key: subjectKey, label: `Step ${i} Subject`,
        column_type: 'text', position: maxPosition, width: 250, is_visible: true, is_enrichment: true,
      })
    }
    ...
    if (!bodyId) {
      maxPosition++
      columnsToCreate.push({
        table_id: tableId, key: bodyKey, label: `Step ${i} Body`,
        column_type: 'text', position: maxPosition, width: 350, is_visible: true, is_enrichment: true,
      })
    }
...
export async function handleEmailSequence(req: Request): Promise<Response> {
  ...
  try {
    ...
    await storeGenerationConfig(
      serviceClient, body.table_id, body.sequence_config, signOff, body.model, numSteps,
    )
    ...
    const writeStepCells = async (prospectRowId: string, steps: EmailStep[]) => {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (colIds.subjectColumnId) {
          cellsToUpsert.push({
            row_id: prospectRowId, column_id: colIds.subjectColumnId,
            value: step.subject, status: 'complete', source: 'enrichment',
          })
        }
        if (colIds.bodyColumnId) {
          cellsToUpsert.push({
            row_id: prospectRowId, column_id: colIds.bodyColumnId,
            value: step.body, status: 'complete', source: 'enrichment',
          })
        }
      }
    ...
    // Tier 1: Claude writes real emails for prospect[0]
    ...
    if (!useGeminiOnly && ANTHROPIC_API_KEY) {
      try {
        ...
        await logAICostEvent(
          serviceClient, user.id, orgId, 'anthropic', 'claude-sonnet-4-5-20250929',
          tier1Result.inputTokens, tier1Result.outputTokens, 'content_generation',
        )
    ...
    // Tier 2: Gemini generates emails for remaining prospects
        ...
        try {
          ...
          if (exampleEmails) {
            steps = await generateEmailsFromExample(
              prospect, firstProspect, exampleEmails,
              { num_steps: numSteps, angle: body.sequence_config.angle, email_type: body.sequence_config.email_type, event_details: body.sequence_config.event_details },
              contextPrompt, signOff,
            )
          } else {
