/**
 * Handler: generate-waitlist-token
...
export async function handleWaitlistToken(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (request.waitlist_entry_id) {
      ...
      if (entryCheckError) {
        ...
        return new Response(
          JSON.stringify({ success: false, error: 'Failed to verify waitlist entry' }),
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: error.message || 'Failed to generate token' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
