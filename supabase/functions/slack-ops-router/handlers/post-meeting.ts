/**
 * Handler: post-meeting
 * Extracted from supabase/functions/slack-post-meeting/index.ts
 */
...
export async function handlePostMeeting(req: Request): Promise<Response> {
    ...
    if ((auth.mode === 'cron' || auth.mode === 'service_role') && !meetingId && !isTest) {
      ...
      for (const o of orgsToProcess) {
        ...
        for (const m of candidates || []) {
          ...
          try {
            const resp = await fetch(`${supabaseUrl}/functions/v1/slack-ops-router`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${supabaseServiceKey}`,
              },
              body: JSON.stringify({ action: 'post_meeting', meetingId: m.id, orgId: oid }),
            });
