/**
 * Handler: expire-actions
 * Extracted from supabase/functions/slack-expire-actions/index.ts
 */
...
export async function handleExpireActions(req: Request): Promise<Response> {
  ...
  if (!isCron && !isServiceRole) {
    if (!cronSecret) {
      console.error('[slack-expire-actions] CRON_SECRET not configured - rejecting request');
