...
async function detectStaleLeads(
  ...
  insights.push({
    ...
    body: `${staleCount} leads haven't been touched in 30+ days. Based on your team's patterns, re-engagement emails within 48 hours can recover 23% of cold leads. Want me to draft re-engagement emails for the top ${Math.min(10, staleCount)}?`,
    actions: [
      {
...
export async function handleInsightsEngine(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Support both direct calls (action) and router calls (handler_action)
    const action = (body.handler_action || body.action) as RequestBody['action'];
