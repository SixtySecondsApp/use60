...
export async function handleAiQuery(req: Request): Promise<Response> {
  ...
  try {
    ...
    const handlerAction = body.handler_action || body.action;
    if (handlerAction === 'save_recipe' && body.saveAsRecipe) {
    ...
    if (handlerAction === 'execute_recipe' && body.recipeId) {
      const { recipeId } = body;
    ...
    switch (toolName) {
      ...
      // OI-021: Cross-table query handler — delegate to cross_query handler via router
      case 'cross_table_query': {
        ...
        const { data: crossResult, error: crossError } = await supabase.functions.invoke(
          'ops-table-router',
          {
            body: { action: 'cross_query', tableId, query: crossQuery, dataSources: target_sources },
          }
        );
