// Handler extracted from cc-action-sync/index.ts
...
export async function handleActionSync(req: Request): Promise<Response> {
  try {
    ...
    // When called via cc-router, the original "action" field is sent as "sub_action"
    // to avoid collision with the router's "action" field.
    const item_id = body.item_id;
    const action = body.sub_action ?? body.action;
    if (!item_id || !action) return errorResponse('item_id and action (or sub_action) are required', req, 400);
