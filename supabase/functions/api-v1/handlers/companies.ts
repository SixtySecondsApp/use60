...
// List companies
export async function handleCompaniesList(client: any, url: URL, userId: string, permissions: any) {
  try {
...
// Get single company
export async function handleSingleCompany(client: any, companyId: string, userId: string, permissions: any) {
  try {
...
// Create company
export async function handleCreateCompany(client: any, body: any, userId: string) {
  try {
...
// Update company
export async function handleUpdateCompany(client: any, companyId: string, body: any, userId: string) {
  try {
    if (!isValidUUID(companyId)) {
      return createErrorResponse('Invalid company ID format', 400, 'INVALID_ID_FORMAT')
    }
...
// Delete company
export async function handleDeleteCompany(client: any, companyId: string, userId: string) {
  try {
    if (!isValidUUID(companyId)) {
      return createErrorResponse('Invalid company ID format', 400, 'INVALID_ID_FORMAT')
    }
