...
export async function handleCreditMenu(req: Request): Promise<Response> {
  ...
  try {
    ...
    // 2. Parse optional tier from query param or body
    const url = new URL(req.url);
    let tierParam = url.searchParams.get('tier');

    // Also check body for tier (when called via router)
    if (!tierParam) {
      try {
        const body = await req.json();
        tierParam = body.tier || null;
      } catch {
