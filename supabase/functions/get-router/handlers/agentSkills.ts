...
export async function handleAgentSkills(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Parse request body
    const requestBody: AgentSkillsRequest & { _skill_action?: string } = await req.json();
    const {
      action: bodyAction,
      _skill_action,
      organization_id,
    ...
    // Resolve skill action: _skill_action (from router) takes precedence,
    // then bodyAction (from direct calls). If bodyAction is the router action name,
    // fall back to 'list'.
    const action: 'list' | 'get' | 'search' = (
      (_skill_action as 'list' | 'get' | 'search') ||
      (['list', 'get', 'search'].includes(bodyAction) ? bodyAction as 'list' | 'get' | 'search' : 'list')
    );
...
async function getSkill(
  ...
  try {
    ...
    const agentSkill: AgentSkill = {
      ...
      kind: (skill.category || 'uncategorized') === 'agent-sequence' ? 'sequence' : 'skill',
    };
