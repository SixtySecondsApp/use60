/**
 * Handler: video_generate
 * Migrated from heygen-video-generate edge function.
 *
 * Modes:
  ...
  try {
    ...
    for (const entry of entries) {
      try {
        ...
        if (insertError) {
          console.error('[heygen-router/video-generate] DB insert error:', insertError);
        }
        ...
        if (!resolvedAudioUrl && clonedVoiceId && elevenLabsApiKey) {
          ...
          if (ttsRes.ok) {
            ...
            console.error('[heygen-router/video-generate] TTS failed:', ttsRes.status);
          }
        }
        ...
        console.error('[heygen-router/video-generate] Video generation error:', err);
    ...
    console.error('[heygen-router/video-generate] Error:', err);
