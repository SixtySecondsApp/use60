/**
 * Handler: admin
 * Migrated from heygen-admin edge function.
 *
 * Sub-actions:
...
interface AdminRequest {
  action: string; // router-level action or sub-action
  sub_action?: 'save_credentials' | 'test_credentials' | 'list_voices' | 'get_account_info';
  api_key?: string;
}
  ...
  try {
    ...
    switch (subAction) {
      case 'save_credentials': {
        ...
        if (upsertError) {
          console.error('[heygen-router/admin] save_credentials error:', upsertError);
      ...
      default:
        return errorResponse(`Unknown admin sub-action: ${subAction}`, req, 400);
    }
  } catch (err) {
    console.error('[heygen-router/admin] Error:', err);
