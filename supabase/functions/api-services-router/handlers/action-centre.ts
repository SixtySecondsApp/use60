/**
 * Action Centre handler — extracted from api-action-centre/index.ts
 */
...
export async function handleActionCentre(req: Request): Promise<Response> {
  ...
  try {
    ...
    const pathParts = url.pathname
      .split('/')
      .filter(s => s && s !== 'functions' && s !== 'v1' && s !== 'api-action-centre' && s !== 'api-services-router');
