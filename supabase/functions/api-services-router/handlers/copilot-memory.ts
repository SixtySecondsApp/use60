/**
 * Copilot Memory handler — extracted from api-copilot-memory/index.ts
 */
...
export async function handleCopilotMemory(req: Request): Promise<Response> {
  ...
  try {
    ...
    const pathParts = url.pathname
      .split('/')
      .filter(s => s && s !== 'functions' && s !== 'v1' && s !== 'api-copilot-memory' && s !== 'api-services-router');
