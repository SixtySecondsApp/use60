...
import {
  ...
  handleUpdateTask,
  handleDeleteTask
} from '../../api-v1/handlers/tasks.ts'

/**
 * Unified handler for the tasks entity.
 *
  ...
  try {
    ...
    if (req.method === 'GET' || subAction === 'list' || subAction === 'get') {
      if (subAction === 'get' && recordId) {
        response = await handleSingleTask(client, recordId, user_id, permissions)
      ...
      if (!recordId) {
        return createErrorResponse('Task ID required', 400, 'MISSING_ID', undefined, req)
      }
      ...
      response = await handleUpdateTask(client, recordId, updateBody, user_id, permissions)
      ...
      response = await handleDeleteTask(client, recordId, user_id, permissions)
    } else {
      if (!checkPermission(permissions, 'tasks:write')) {
        return createErrorResponse('Write permission required', 403, 'INSUFFICIENT_PERMISSIONS', undefined, req)
      }
      const { sub_action: _sa, action: _a, ...createBody } = body || {}
      response = await handleCreateTask(client, createBody, user_id)
    }
    ...
    if (apiKey) {
      logApiUsage(client, apiKey, '/api-v1-router/tasks', req.method, statusCode, user_id, req)
        .catch(console.error)
    }
    ...
    await captureException(error, {
      tags: { function: 'api-v1-router', handler: 'tasks', integration: 'supabase' },
    })
