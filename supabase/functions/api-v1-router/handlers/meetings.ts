...
import {
  ...
  handleUpdateMeeting,
  handleDeleteMeeting
} from '../../api-v1/handlers/meetings.ts'

/**
 * Unified handler for the meetings entity.
 *
 * sub_action values: list | get | create (default) | update | delete
 */
export async function handleMeetings(req: Request): Promise<Response> {
  let statusCode = 200
  ...
  try {
    ...
    if (req.method === 'GET' || subAction === 'list' || subAction === 'get') {
      if (subAction === 'get' && recordId) {
        response = await handleSingleMeeting(client, recordId, user_id, permissions)
      ...
      if (!recordId) {
        return createErrorResponse('Meeting ID required', 400, 'MISSING_ID', undefined, req)
      }
      ...
      response = await handleUpdateMeeting(client, recordId, updateBody, user_id, permissions)
      ...
      response = await handleDeleteMeeting(client, recordId, user_id, permissions)
    } else {
      if (!checkPermission(permissions, 'meetings:write')) {
        return createErrorResponse('Write permission required', 403, 'INSUFFICIENT_PERMISSIONS', undefined, req)
      }
      const { sub_action: _sa, action: _a, ...createBody } = body || {}
      response = await handleCreateMeeting(client, createBody, user_id)
    }
    ...
    if (apiKey) {
      logApiUsage(client, apiKey, '/api-v1-router/meetings', req.method, statusCode, user_id, req)
        .catch(console.error)
    }
    ...
    await captureException(error, {
      tags: { function: 'api-v1-router', handler: 'meetings', integration: 'supabase' },
    })
