/**
 * Handler: process_structured_summary
 * Extracted from meeting-process-structured-summary
 *
...
function extractDealTruthFromSummary(summary: StructuredSummary): DealTruthExtraction[] {
  ...
  if (summary.outcome_signals.next_steps && summary.outcome_signals.next_steps.length > 0) {
    ...
    extractions.push({
      ...
      next_step_date: dateMatch ? undefined : undefined,
    });
  }
...
export async function handleProcessStructuredSummary(req: Request): Promise<Response> {
