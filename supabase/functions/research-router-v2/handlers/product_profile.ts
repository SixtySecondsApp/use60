// Handler: research-product-profile (consolidated into research-router-v2)
...
export async function handleProductProfile(req: Request): Promise<Response> {
  ...
  try {
    ...
    // ------------------------------------------------------------------
    const body = (await req.json()) as ResearchRequest & { sub_action?: string }
    const { product_profile_id } = body
    // When called via research-router-v2, the internal action is in sub_action;
    // body.action is the router-level action ('product_profile').
    const action = (body.sub_action ?? body.action) as ResearchRequest['action']
