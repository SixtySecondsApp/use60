...
export async function handleMemory(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Support both URL path routing (original) and body sub_action (via router)
    const path = (body as Record<string, unknown>).sub_action as string || url.pathname.split('/').pop();
