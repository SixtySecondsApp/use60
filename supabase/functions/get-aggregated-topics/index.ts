/**
 * Get Aggregated Topics Edge Function
 *
 * Query API for global topics with full filtering capabilities.
 * Supports date ranges, company/contact filters, search, pagination,
 * and semantic search using Google File Search.
 *
 * Features:
 * - Full filtering: date, company, contact, meeting type, tags
 * - Semantic search via Google File Search
 * - Multiple sort options: frequency, recency, relevance
 * - Pagination with cursor support
 * - Source meeting details on demand
 *
 * API: POST /get-aggregated-topics
 * Body: {
 *   filters?: {
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // First, get global topics that match the base criteria
    ...
    for (const topic of globalTopics || []) {
      ...
      // Skip topics with no matching sources after filtering
      if (!sources || sources.length === 0) {
        continue
      }

      // Get unique company and contact IDs
      ...
      const enrichedTopic: GlobalTopicResponse = {
        ...
        source_count: sources.length, // Use filtered count
      ...
      if (include_sources) {
        // Fetch meeting details for sources
        ...
        // Get company and contact name maps
    ...
    // Get total unique companies and contacts across all user's topics
