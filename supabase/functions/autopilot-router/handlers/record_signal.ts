/**
 * autopilot-router/handlers/record_signal.ts
 *
 * Extracted from autopilot-record-signal/index.ts (AP-010 / AP-032).
 ...
 * GET  — return + clear pending promotion nudge (AP-032)
 */
...
async function countCleanApprovals(
  ...
  if (error) {
    console.error('[autopilot-router:record_signal] countCleanApprovals error:', error)
    return 0
  }
 ...
 * promotion-eligible, write a nudge.
 *
...
async function maybeSetPromotionNudge(
  ...
  try {
    ...
    console.error('[autopilot-router:record_signal] maybeSetPromotionNudge error:', err)
  }
}
...
async function handleGet(req: Request): Promise<Response> {
  ...
  if (fetchError) {
    console.error('[autopilot-router:record_signal] GET nudge fetch error:', fetchError)
  ...
  if (clearError) {
    console.error('[autopilot-router:record_signal] GET nudge clear error:', clearError)
...
async function handlePost(req: Request): Promise<Response> {
  ...
  if (membershipError) {
    console.error('[autopilot-router:record_signal] org membership lookup error:', membershipError)
  ...
  if (insertError || !inserted) {
    console.error('[autopilot-router:record_signal] insert error:', insertError)
  ...
  if (rubber_stamp) {
    serviceClient
      ...
      .then(({ error }) => {
        if (error) {
          console.error('[autopilot-router:record_signal] increment_rubber_stamp_count error:', error)
        }
      })
      .catch((err) => {
        console.error('[autopilot-router:record_signal] increment_rubber_stamp_count unexpected error:', err)
      })
  }
  ...
  recalculateUserConfidence(serviceClient, userId, orgId, action_type).catch((err) => {
    console.error('[autopilot-router:record_signal] recalculateUserConfidence error:', err)
  })
  ...
  updateRepMemory(serviceClient, userId, orgId).catch((err) => {
    console.error('[autopilot-router:record_signal] updateRepMemory error:', err)
  })
  ...
  if (signal === 'undone' || signal === 'auto_undone') {
    evaluateDemotionTriggers(serviceClient, userId, orgId, action_type)
      ...
      .catch((err) => {
        console.error('[autopilot-router:record_signal] demotion evaluation error:', err)
      })
  }
  ...
  const stepLog = Object.entries(perf.steps)
    .map(([k, ms]) => `${k}=${ms}ms`)
    .join(' ')
  console.log(`[autopilot-router:record_signal] perf total=${total}ms ${stepLog}`)
  if (total > 200) {
    console.warn(`[autopilot-router:record_signal] SLOW REQUEST: ${total}ms (threshold=200ms)`)
  }
...
export async function handleRecordSignal(req: Request): Promise<Response> {
  if (req.method === 'GET') {
