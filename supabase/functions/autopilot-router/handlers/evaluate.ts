/**
 * autopilot-router/handlers/evaluate.ts
 *
 * Extracted from autopilot-evaluate/index.ts (AP-014).
 ...
 * engine, and sends a Slack proposal for every qualifying candidate.
 */
...
async function resolveAuth(
  ...
  if (membershipError) {
    console.error('[autopilot-router:evaluate] membership lookup error:', membershipError)
...
async function runEvaluation(
  ...
  if (eligibleError) {
    console.error('[autopilot-router:evaluate] Failed to fetch eligible rows:', eligibleError)
    throw `Database error fetching eligible rows: ${eligibleError.message}`
  }

  const rows: EligibleRow[] = (eligibleRows ?? []) as EligibleRow[]
  console.log(
    `[autopilot-router:evaluate] Found ${rows.length} promotion-eligible (user, action_type) row(s)` +
  ...
  for (const row of rows) {
    ...
    try {
      ...
      console.error(
        `[autopilot-router:evaluate] Error evaluating user=${row.user_id} action_type=${row.action_type}:`,
        err,
      )
    }
  }
  ...
  console.log(
    `[autopilot-router:evaluate] Evaluation complete: ${evaluatedUsers} rows checked, ` +
  ...
  // ---------------------------------------------------------------------------
  // Step 4 — Send a proposal for each user.
  // ---------------------------------------------------------------------------
  let proposalsSent = 0
  ...
  for (const [userId, { orgId: userOrgId, candidates }] of byUser) {
    try {
      ...
      console.error(
        `[autopilot-router:evaluate] Error sending proposal for user=${userId}:`,
        err,
      )
    }
  }
  ...
  // ---------------------------------------------------------------------------
  // Step 5 — Reset promotion_eligible = false on rows whose proposals were sent.
  // ---------------------------------------------------------------------------
  if (sentUserIds.size > 0) {
    ...
    const resetPromises = candidatesToReset.map((candidate) =>
      serviceClient
        ...
        .then(({ error }) => {
          if (error) {
            console.error(
              `[autopilot-router:evaluate] Failed to reset promotion_eligible for ` +
        ...
        .catch((err) => {
          console.error(
            `[autopilot-router:evaluate] Unexpected error resetting promotion_eligible for ` +
            `user=${candidate.user_id} action_type=${candidate.action_type}:`,
            err,
          )
        }),
    ...
    console.log(
      `[autopilot-router:evaluate] Reset promotion_eligible=false for ` +
...
export async function handleEvaluate(req: Request): Promise<Response> {
  ...
  try {
    ...
    // -------------------------------------------------------------------------
    console.log(
      `[autopilot-router:evaluate] Starting evaluation: org_id=${org_id ?? 'all'} dry_run=${dry_run}`,
    )

    const result = await runEvaluation(serviceClient, org_id, dry_run)

    console.log(
      `[autopilot-router:evaluate] Complete: evaluated_users=${result.evaluated_users} ` +
      `candidates_found=${result.candidates_found} proposals_sent=${result.proposals_sent}`,
    )

    return jsonResponse(result, req, 200)
  } catch (err) {
    console.error('[autopilot-router:evaluate] Unexpected error:', err)
