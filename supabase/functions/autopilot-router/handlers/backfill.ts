/**
 * autopilot-router/handlers/backfill.ts
 *
 * Extracted from backfill-runner/handlers/autopilot.ts (AP-012).
 * Admin-only function that imports historical approval data from
...
async function resolveAuth(
  ...
  if (membershipError) {
    console.error('[autopilot-router:backfill] membership lookup error:', membershipError)
...
async function runBackfill(
  ...
  if (fetchError) {
    console.error('[autopilot-router:backfill] Failed to fetch slack_pending_actions:', fetchError)
    throw `Database error fetching pending actions: ${fetchError.message}`
  }

  const rows = (pendingActions ?? []) as SlackPendingAction[]
  const total = rows.length
  console.log(`[autopilot-router:backfill] Processing ${total} record(s) for org ${orgId} (dry_run=${dryRun})`)
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (i > 0 && i % 100 === 0) {
      console.log(`[autopilot-router:backfill] Processing ${i} of ${total} records...`)
    }
    ...
    try {
      ...
      if (!signal) {
        console.warn(`[autopilot-router:backfill] Skipping row ${row.id} — unmappable status: ${row.status}`)
        skipped++
        continue
      }
      ...
      if (existingError) {
        console.error(`[autopilot-router:backfill] Duplicate check error for row ${row.id}:`, existingError)
        errors++
        continue
      }
      ...
      if (insertError) {
        console.error(`[autopilot-router:backfill] Insert error for row ${row.id}:`, insertError)
        errors++
        continue
      }
      ...
      console.error(`[autopilot-router:backfill] Unexpected error processing row ${row.id}:`, err)
      errors++
    }
  }
  ...
  if (!dryRun && recalcSet.size > 0) {
    console.log(`[autopilot-router:backfill] Recalculating confidence for ${recalcSet.size} (user, action_type) pair(s)...`)
    ...
    for (const key of recalcSet) {
      ...
      recalcPromises.push(
        recalculateUserConfidence(serviceClient, userId, orgId, actionType).catch((err) => {
          console.error(`[autopilot-router:backfill] recalculateUserConfidence error for ${key}:`, err)
        }),
      )
    }

    await Promise.all(recalcPromises)
    console.log(`[autopilot-router:backfill] Confidence recalculation complete.`)
  }
...
export async function handleBackfill(req: Request): Promise<Response> {
  ...
  try {
    ...
    // -------------------------------------------------------------------------
    console.log(
      `[autopilot-router:backfill] Starting backfill: org_id=${org_id} dry_run=${dry_run} limit=${limit} after_date=${after_date ?? 'all'}`,
    )

    const result = await runBackfill(serviceClient, org_id, dry_run, limit, after_date)

    console.log(
      `[autopilot-router:backfill] Complete: processed=${result.processed} skipped=${result.skipped} errors=${result.errors}`,
    )

    return jsonResponse(result, req, 200)
  } catch (err) {
    console.error('[autopilot-router:backfill] Unexpected error:', err)
