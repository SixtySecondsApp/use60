...
/**
 * Bullhorn OAuth Initiation (org-scoped)
 *
 * Flow:
 *  - Frontend calls this function with Authorization header (user session JWT)
 *  - We validate user + org admin role
 *  - We create bullhorn_oauth_states row (CSRF protection)
 *  - We return Bullhorn authorize URL for the user to complete OAuth
 */
export async function handleInitiate(req: Request): Promise<Response> {
  if (req.method === 'OPTIONS') return new Response('ok', { headers: corsHeaders })

  try {
    // Optional JSON body: { org_id?: string, redirect_path?: string }
    ...
    if (existing?.id && existing.is_connected) {
      return new Response(
        JSON.stringify({
          ...
          message: 'This organization already has an active Bullhorn connection. Disconnect first to reconnect.',
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    if (!clientId || !redirectUri) {
      return new Response(JSON.stringify({ success: false, error: 'Missing BULLHORN_CLIENT_ID or BULLHORN_REDIRECT_URI' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { error: stateError } = await supabase.from('bullhorn_oauth_states').insert({
      state,
      user_id: user.id,
      org_id: orgId,
    ...
    // Build Bullhorn authorization URL
    ...
    // Optionally set action=Login to force login prompt
