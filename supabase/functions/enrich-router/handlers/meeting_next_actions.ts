...
export async function handleMeetingNextActions(req: Request): Promise<Response> {
  ...
  try {
    ...
    const { table_id, batch_size, sub_action } = body as Record<string, unknown>;
    const action = (sub_action as string) || (body.action !== 'meeting_next_actions' ? body.action as string : 'enrich');
