...
serve(async (req) => {
  ...
  try {
    ...
    // --- Call Gemini 3.1 Pro ---
    // No artificial timeout — let the request run up to the Supabase wall-clock
    // limit (150s free / 400s pro). Server-side retry removed to avoid doubling
    // execution time; the client retries failed requests instead.
