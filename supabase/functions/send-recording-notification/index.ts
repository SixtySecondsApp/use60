/**
 * Send Recording Notification Edge Function
 *
 * Sends Slack notifications for recording events:
 * - Bot joining meeting
 * - Bot failed to join
 * - Recording ready with summary
 * - HITL deal selection required
 * - HITL speaker confirmation required
 *
 * Endpoint: POST /functions/v1/send-recording-notification
 */
...
/**
 * Get Slack bot token for organization
 */
...
/**
 * Get recording details for notification
 */
...
async function sendNotification(
  ...
  // Send notification
  // If channel is configured, post to channel
  // Otherwise, send as DM to user
  if (settings.channel_id) {
    ...
    if (slackUserId) {
      // Use DM channel (user ID works as channel for DMs)
