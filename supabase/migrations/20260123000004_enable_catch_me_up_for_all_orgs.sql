-- Enable: Catch Me Up Sequence for all orgs
-- Date: 2026-01-23
-- Story: US-004
--
-- Uses the same pattern as 20260114000023_enable_remaining_sequences_for_all_orgs.sql
-- Inserts organization_skills rows for every org

BEGIN;

-- Enable seq-catch-me-up for all orgs
WITH skills AS (
  SELECT
    ps.id AS platform_skill_id,
    ps.skill_key,
    ps.version AS platform_skill_version,
    COALESCE(ps.frontmatter->>'name', 'Catch Me Up') AS skill_name
  FROM platform_skills ps
  WHERE ps.skill_key = 'seq-catch-me-up'
    AND ps.is_active = true
    AND ps.category = 'agent-sequence'
)
...
-- Also enable the daily-brief-planner skill
WITH skills AS (
  SELECT
    ps.id AS platform_skill_id,
    ps.skill_key,
    ps.version AS platform_skill_version,
    COALESCE(ps.frontmatter->>'name', 'Daily Brief Planner') AS skill_name
  FROM platform_skills ps
  WHERE ps.skill_key = 'daily-brief-planner'
    AND ps.is_active = true
)
INSERT INTO organization_skills (
  organization_id,
