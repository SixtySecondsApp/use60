-- Add Output Format Selector SKILL
-- Date: 2026-01-24
-- Story: FOUND-001
-- Purpose: Create a skill that helps the agent select optimal structured response formats
--          based on user intent and available data

BEGIN;

-- Insert the output-format-selector skill
...
VALUES (
  ...
  '{
    ...
    "requires_context": ["user_intent", "available_data"],
    "outputs": ["response_type", "response_config"],
    "triggers": ["any_copilot_response"],
    "priority": "system"
  }'::jsonb,
  E'# Output Format Selector SKILL
