-- Update approve_join_request to mark onboarding as complete when membership is created
-- This prevents approved users from being redirected back to onboarding flow
...
BEGIN
  ...
  IF EXISTS (
    ...
    -- Mark onboarding as complete even for existing members
  ...
  -- Mark onboarding as complete atomically with membership creation
  -- This ensures approved users don't get redirected back to onboarding
  INSERT INTO user_onboarding_progress (
    user_id,
    onboarding_step,
    onboarding_completed_at
...
COMMENT ON FUNCTION "public"."approve_join_request"(p_request_id uuid) IS 'Approves a join request and creates organization membership. Ensures user profile has email and marks onboarding as complete atomically to prevent redirect loops.';
