-- Migration: Link leads to meetings + track meeting outcome
-- 1. Add meeting_id FK and meeting_outcome to leads table
-- 2. Add meeting_outcome column to standard Leads ops table
-- 3. Update provision RPC with new columns
-- 4. Add view for no-shows/reschedules

-- =============================================================================
-- 1. Add meeting_id and meeting_outcome to leads table
-- =============================================================================
...
ALTER TABLE public.leads
  ...
  ADD COLUMN IF NOT EXISTS meeting_outcome TEXT NOT NULL DEFAULT 'scheduled'
    CHECK (meeting_outcome IN ('scheduled', 'completed', 'no_show', 'rescheduled', 'cancelled'));

-- Index for fast lookup of leads by meeting
CREATE INDEX IF NOT EXISTS idx_leads_meeting_id ON public.leads(meeting_id) WHERE meeting_id IS NOT NULL;

-- Index for filtering by outcome
CREATE INDEX IF NOT EXISTS idx_leads_meeting_outcome ON public.leads(meeting_outcome);

COMMENT ON COLUMN public.leads.meeting_id IS 'FK to the actual meetings row — links the booked lead to the recorded meeting';
COMMENT ON COLUMN public.leads.meeting_outcome IS 'Whether the meeting went ahead: scheduled (pending), completed, no_show, rescheduled, cancelled';

-- =============================================================================
-- 2. Add meeting_outcome column to existing standard Leads tables
-- =============================================================================
...
SELECT
  ...
  'meeting_outcome', 'Meeting Outcome', 'status', 10, 160,
  'leads', 'meeting_outcome',
  '[{"value":"scheduled","label":"Scheduled","color":"blue"},{"value":"completed","label":"Completed","color":"green"},{"value":"no_show","label":"No Show","color":"red"},{"value":"rescheduled","label":"Rescheduled","color":"yellow"},{"value":"cancelled","label":"Cancelled","color":"gray"}]'::jsonb,
  true, true, true
FROM dynamic_tables t
WHERE t.is_standard = true AND t.name = 'Leads'
  AND NOT EXISTS (
    SELECT 1 FROM dynamic_table_columns c
    WHERE c.table_id = t.id AND c.key = 'meeting_outcome'
  );

-- Shift created_at position to make room (was 9, now 11)
...
-- =============================================================================
-- 3. Update provision RPC with meeting_outcome column
-- =============================================================================
...
BEGIN
  ...
  -- =========================================================================
  -- LEADS TABLE (11 columns — sourced from leads table)
  -- =========================================================================
  ...
  INSERT INTO dynamic_table_columns (
    ...
    (gen_random_uuid(), v_leads_id, 'meeting_outcome', 'Meeting Outcome', 'status', 9, 160, 'leads', 'meeting_outcome', '[{"value":"scheduled","label":"Scheduled","color":"blue"},{"value":"completed","label":"Completed","color":"green"},{"value":"no_show","label":"No Show","color":"red"},{"value":"rescheduled","label":"Rescheduled","color":"yellow"},{"value":"cancelled","label":"Cancelled","color":"gray"}]'::jsonb, true, true, true),
    (gen_random_uuid(), v_leads_id, 'created_at', 'Created', 'date', 10, 160, 'leads', 'created_at', NULL, true, true, true);
  ...
  -- =========================================================================
  -- MEETINGS TABLE (11 columns — app-sourced, no hubspot)
  -- =========================================================================
  INSERT INTO dynamic_tables (
    id, organization_id, created_by, name, source_type, is_standard, description
  ) VALUES (
  ...
  -- =========================================================================
  -- ALL COMPANIES TABLE (12 columns)
  -- =========================================================================
  INSERT INTO dynamic_tables (
    id, organization_id, created_by, name, source_type, is_standard, description
  ) VALUES (
