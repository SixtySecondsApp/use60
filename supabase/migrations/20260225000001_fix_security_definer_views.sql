...
-- =============================================================================
-- 8 views were flagged as SECURITY DEFINER, running with the view creator's
-- permissions rather than the querying user's permissions.
--
-- Fix strategy:
--   Group 1 (5 views): Recreate with security_invoker = on
--     - subscription_facts_view
--     - mrr_current_view
--     - mrr_movement_view
--     - meeting_action_items_view
--     - next_action_suggestions_view
--
--   Group 2 (3 admin-only PII views): Recreate with security_invoker = on
--     AND revoke SELECT from anon/authenticated, grant only to service_role
--     - users_without_organizations
--     - affected_personal_email_users
--     - single_member_personal_email_orgs
-- =============================================================================

-- ─── Group 1: Apply security_invoker = on ────────────────────────────────────

-- 1. subscription_facts_view (mrr_current_view depends on this, recreate first)
...
-- 2. mrr_current_view (depends on subscription_facts_view)
...
-- These views expose cross-org user data and are intended for internal/admin
-- use only. Recreate with security_invoker=on and restrict access to service_role.
